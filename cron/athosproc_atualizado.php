...
if ($iRemessa906in == $iRemessa906out) {
	$aListaProposta = $oProposta->getListaProposta(false, false, true);
...
$i=1;
		...
		/**
             ...
             */
            $db->query="Select * from intvquitante where cod_ppst='".$aDadoListaProposta["cod_ppst"]."'";
            $db->query();
            $saldo_iq = 0;
            $saldo_iq_por_proponente = 0;
            if($db->qrcount>0) {
                $saldo_iq = $db->qrdata[0]['VLSALDODEV_INTQ'];
                $saldo_iq_por_proponente = $saldo_iq;
                if ($saldo_iq > 0) {
                    $saldo_iq_por_proponente = $saldo_iq / count($aDadosProposta['proponentes']);
                }
				...
				$numppst=$db->qrcount;
						...
						{	
							...
							{ 
								...
								$aBufferInterveniente .= $oArquivo->formatNumber($aDadosProponente["vlfinsol_ppnt"], 15,"0"," ");
							}
						}
