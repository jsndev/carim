...
//				$sBuffer .= $oArquivo->formatDate($aDadosImovel["dtaprovacao_imov"]); 			// 08 - Data de aprovacao do imovel
				...
				if ($aDadosDevSolid["nome_devsol"] && 
					...
					$sBuffer .= $oArquivo->formatNumber($aDadosDevSolid["cod_pais"],6,"0"," ");
				...
				$sBufferFgts .=$oArquivo->formatNumber($aDadosProponente["listadenomes"][0]["id_lstn"], 9)."\n";
				$sBuffer .= $sBuffer.$BufferFgts; 
				$contadorProponente++;
...
$db->query = "Select valorcompra_ppst from proposta where cod_ppst='".$cod_ppst."'";
$db->query();
if($db->qrcount>0)
{
		$valorcompra=$db->qrdata[0]['valorcompra_ppst'];
}

$query="Select * from fgts where cod_usua='".$cod_usua."'";	
$result =mysql_query($query);
...
$registro = mysql_fetch_array($result, MYSQL_ASSOC);
	...
	$aAltPpnt["fgts"][0]["valoper_fgts"]=		$registro['VALOPERACAO'];
...
$aBufferFgts .= $oArquivo->formatNumber($aAltPpnt["fgts"][0]["valoper_fgts"],15,"0"," ");
...
if($db->qrcount>0)
{
	...
	while($c<$db->qrcount)
	{
		...
		$aBufferContas .=	$oArquivo->formatNumber($db->qrdata[$c]['SUREG'],2);
		$aBufferContas .=	 "\n";
		$c++;
	}
}*/
			...
			/// Trailer do arquivo
			$sBufferTrailer  = "99";
			$sBufferTrailer .= $oArquivo->formatNumber(($iNumRegistro+2+$fgts+$cfgts),10);
