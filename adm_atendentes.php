...
if ($_GET["dmy"] != "" && $_GET["cod_usua"] != "" && $_GET["ac"] == "del") {
	...
	if ($oUsuario->getErrNo() == 0) {
		$oMensagem->setMensagem("O atendente foi removido com sucesso", MSG_SUCESSO);
	} else {
		$oMensagem->setMensagem("O atendente não pôde ser removido. Tente novamente.", MSG_ERRO);
	}
	...
	if ($oUsuario->getErrNo() == 0) {
		...
		$email->send();
		$oMensagem->setMensagem("A senha do atendente \"".$dadosUsuario[0]["nome_usua"]."\" foi alterada com sucesso. Um e-mail foi enviado ao seu endereço eletrônico contendo sua nova senha de acesso.", MSG_SUCESSO);
	} else {
		$oMensagem->setMensagem("A senha do atendente \"".$dadosUsuario[0]["nome_usua"]."\" não pôde ser alterada. Tente novamente.", MSG_ERRO);
	}
}

$aListaUsuario = $oUsuario->getListaUsuarios(TPUSER_ATENDENTE);
?>
<script language="javascript" type="text/javascript" src="js/cadastroAtendente.js"></script>
...
?>
				...
				<tfoot>
					<tr>
						<td colspan="5"><a href="adm_atendentes_add.php"><img src="images/buttons/bt_adicionar.gif" alt=" " /></a></td>
					</tr>
				</tfoot>
			</table>
		</div>
