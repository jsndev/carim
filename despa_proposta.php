...
if($acaoProposta=='salvar'){
	foreach ($_POST['ckl_doc_dt_ped'] as $key=>$value) {
		...
		$c_dt_ped = $c_check ? $c_dt_ped : 'NULL';
		$c_dt_emis = $c_check ? $c_dt_emis : 'NULL';
		...
		$db->query="replace into checklist(
						...
						'".mysql_real_escape_string($_GET['cod_proposta'])."',
						'".mysql_real_escape_string($_POST['f_i_cidade_cod'])."',
						'".mysql_real_escape_string($_POST['f_i_uf_cod'])."',
						".$c_dt_ped.",
						".$c_dt_emis.",
...
if(empty($_GET["cod_proposta"])){
	header('Location:despa_lista_prop.php'); exit();
}
...
$db->query="SELECT
            ...
            WHERE u.cod_usua = p.proponente_ppst
              ...
              AND p.cod_ppst = i.cod_ppst
            	...
            	AND p.despachante_ppst='".mysql_real_escape_string($cLOGIN->iID)."'
...
<script language="JavaScript" src="./js/despachante.js"></script>
...
<form name="proposta" id="proposta" method="post" action="<?=$php_self;?>">
  ...
  ?>
  <input type="hidden" name="f_i_cidade_cod" value="<?=$f_i_cidade_cod;?>">
  <input type="hidden" name="f_i_uf_cod" value="<?=$f_i_uf_cod;?>">
  <a name="checklist"></a>
  <br><b>Check List</b>
	<div class="quadroInterno">
		<div><img src="images/layout/subquadro_t.gif" alt=" " /></div>
