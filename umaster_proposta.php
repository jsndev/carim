...
$aProposta     = $oProposta->getProposta($_GET['cod_proposta']);

$acaoProposta = $crypt->decrypt($_POST["acaoProposta"]);


if($acaoProposta=='status'){
	// novo evento // -------------------------------------------------------------
	$f_novo_evento = $_POST['novo_evento'];
	$f_sit_antiga = $_POST['sit_antiga'];
	$f_sit_nova = $_POST['sit_nova'];

	$s_txt_situacao = "Alterado situação da proposta.<br>\nDe: ".$_SESSION["prop_status"][$f_sit_antiga]." Para: ".$_SESSION["prop_status"][$f_sit_nova].".<br>\nMotivo: ".htmlentities($f_novo_evento);
	
	$cLOGIN->insert_log(4,7,'Alteração de Situação - CODPPST:'.$_GET['cod_proposta']);
	$cLOGIN->insert_history($_GET['cod_proposta'],2,$s_txt_situacao);

	$db->query="update 
				  proposta
				set
	              situacao_ppst='".mysql_real_escape_string($f_sit_nova)."'
	            WHERE 
	              cod_ppst = '".mysql_real_escape_string($_GET['cod_proposta'])."' ";
	$db->query();
	// ----------------------------------------------------------------------------
}
...
*/
	?>
		...
		<div class="quadroInterno">
	        ...
	        <table cellpadding=0 cellspacing=5 border=0>
	          <tr>
	            ...
	            <td align="left"><b>R$ <?=$utils->formataMoeda($aProposta["vlmaxfinan"]);?></b></td>
	          </tr>
	          <tr>
	            <td align="right">Valor Máximo por Prestação:</td>
	            <td align="left"><b>R$ <?=$utils->formataMoeda($aProposta["parcmaxfinan"]);?></b></td>
	          </tr>
	          <tr>
	            <td align="right">Prazo Máximo do Financiamento:</td>
	            <td align="left"><b><?=$utils->formataMoeda($aProposta["przmaxfinan"]);?> meses</b></td>
	          </tr>
	        </table>
			</div>
		...
		<div class="quadroInterno">
			...
			<div class="quadroInternoMeio">
	    		...
	    		<table cellpadding=0 cellspacing=5 border=0>
		  			...
		  			<tr>
		  			  <td align="right" valign="top">FGTS:</td><td align="left"><b>R$ <?=$utils->formataMoeda($aProposta["valorfgts_ppst"]);?></b></td>
		  			</tr>
		  			<tr>
		  			  <td align="right" valign="top">Financiamento:</td><td align="left"><b>R$ <?=$utils->formataMoeda($aProposta["vlfinsol_ppst"]);?></b></td>
		  			</tr>
		  			<tr style="display:none;">
		  			  <td align="right" valign="top">Taxa de Juros:</td><td align="left"><b><?=$utils->formataFloat($aProposta["taxajuros_ppst"],2);?>%</b></td>
		  			</tr>
		  			<? if($aProposta["vlprestsol_ppst"]!=0){ ?>
		  			<tr>
		  			  <td align="right" valign="top">Prestação:</td><td align="left"><b>R$ <?=$utils->formataMoeda($aProposta["vlprestsol_ppst"]);?></b></td>
		  			</tr>
		  			<? } ?>
		  			<? if($aProposta["przfinsol_ppst"]!=0){ ?>
		  			<tr>
		  			...
		  			<tr>
		  			  <td align="right" valign="top">Taxa Manutenção:</td><td align="left"><b>R$ <?=$utils->formataMoeda($aProposta["valormanutencao_ppst"]);?></b></td>
		  			</tr>
	    		</table>
	    	</div>
				<? if ($resultMessage) { ?>
			...
			<div class="quadroInternoMeio">
		    		<table cellpadding=0 cellspacing=5 border=0 width="100%">
		    			...
		    			<tr>
		    			  ...
		    			  <td align="left"  valign="top"><b>R$ <?=$utils->formataMoeda($aProposta["valorboletoaval_ppst"]);?></b></td>
		    			</tr>
		    			<tr>
		    			  <td align="right" valign="top">Data de Pagamento:</td>
		    			  <td align="left"  valign="top"><b><?=$utils->formataDataBRA($aProposta["dtpagtoboleto_ppst"]);?></b></td>
		    			</tr>
		    		</table>
			</div>
			<div><img src="images/layout/subquadro_b.gif" alt=" " /></div>
	  ...
	  <?
	    ### DADOS DO PROPONENTE ##################################################################### 
		  $f_idade = '';
		  if($aProposta["dtnascimento_ppnt"]!=''){ $f_idade = '('.$utils->idade($utils->formataDataBRA($aProposta["dtnascimento_ppnt"])).' anos)'; }
		  
		  $aEndereco = array();
		  $aEndereco["logr"]      = $aProposta["logr_ppnt"];
		  $aEndereco["rua"]       = $aProposta["endereco_ppnt"];
		  $aEndereco["num"]       = $aProposta["nrendereco_ppnt"];
		  $aEndereco["compl"]     = $aProposta["cpendereco_ppnt"];
		  $aEndereco["cep"]       = $utils->formataCep($aProposta["cep_ppnt"]);
		  $aEndereco["bairro"]    = $aProposta["bairro_ppnt"];
		  $aEndereco["uf"]        = $aProposta["uf_ppnt"];
		  $aEndereco["municipio"] = $aProposta["municipio_ppnt"];
	  ?>
	  <br><b>Dados do Proponente</b>
		<div class="quadroInterno">
			...
			<div class="quadroInternoMeio">
		    		<table cellpadding=0 cellspacing=5 border=0>
		    			<tr>
		    			  <td align="right" valign="top">Nome:</td><td align="left"><b><?=$aProposta["nome_usua"];?></b></td>
		    			</tr>
		    			<tr>
		    			  <td align="right" valign="top">Matricula:</td><td align="left"><b><?=$aProposta["id_lstn"];?></b></td>
		    			</tr>
		    			<tr>
		    			  <td align="right" valign="top">E-Mail:</td><td align="left"><b><?=$aProposta["email_usua"];?></b></td>
		    			</tr>
		    			<tr>
		    			  <td align="right" valign="top">CPF:</td><td align="left"><b><?=$aProposta["cpf_ppnt"];?></b></td>
		    			</tr>
		    			<tr>
		    			  <td align="right" valign="top">Data Nasc:</td><td align="left"><b><?=$utils->formataDataBRA($aProposta["dtnascimento_ppnt"]);?> <?=$f_idade;?></b></td>
		    			</tr>
		    			<tr>
		    			  <td align="right" valign="top">Est Civil:</td><td align="left"><b><?=$aProposta["estciv_ppnt"];?></b></td>
		    			</tr>
		    			<tr>
		    			  <td align="right" valign="top">Endereço:</td><td align="left"><b><?=$utils->formataEndereco($aEndereco);?></b></td>
		    			</tr>
			  			<tr>
			  			  <td align="right" valign="top">Telefone:</td><td align="left"><b><?=$utils->formataTelefone($aProposta["telefone_ppnt"]);?></b></td>
			  			</tr>
		    		</table>
			</div>
			<div><img src="images/layout/subquadro_b.gif" alt=" " /></div>
	  ...
	  <br><b>Dados do Imóvel</b>
    		...
    		<table cellpadding=0 cellspacing=5 border=0><tr>
    			<td valign="top">
		    		...
		    		<? /*
		    			...
		    			<tr>
		    			  <td align="right" valign="top">Avaliação do imóvel:</td><td align="left"><b>R$ <?=$imov_vl_aval;?></b></td>
		    			</tr>
		    			<tr>
		      ...
		      <table cellpadding=0 cellspacing=5 border=0>
		        <tr>
		          ...
		          <td align="left"  valign="top">
		            <input type="text" style="width:80px;" name="imov_dt_regist" id="imov_dt_regist" value="<?=$imov_dt_regist;?>" onKeyDown="return teclasInt(this,event);" onKeyUp="return mascaraData(this,event);" maxlength="10">
		          </td>
		        </tr>
	    ...
	    ### DADOS DO VENDEDOR ##################################################################### 
		  ...
		  if($db->qrcount>0){
			  ...
			  if($db->qrdata[0]['COD_ESTCIV']) $cecivil    = $forms->getECivil($db->qrdata[0]['COD_ESTCIV']);
			  ...
			  if($db->qrdata[0]['COD_PROF']) $cprof      = $forms->getProfissao($db->qrdata[0]['COD_PROF']);
			  ...
			  if($vf_nasc) $vf_idade        = '('.$utils->idade($vf_nasc).' anos)';
			  ...
			  if($db->qrdata[0]['COD_CNAE']) $ccnae       = $forms->getAtvEcon($db->qrdata[0]['COD_CNAE']);
    ...
    $db->query="select 
    			...
    			where 
    			  ...
    			  and
    				b.flgstatus_mndc = 1
    			  and
    				b.cod_municipio='".mysql_real_escape_string($i_cod_mun)."'
    			  and
  ...
  <?
		$db->query="SELECT h.cod_ppst, h.obs_hist, h.tipo_hist, u.level_usua, u.nome_usua, '' as cod_chat, h.dt_hist as data
								...
								WHERE p.proponente_ppst = '".mysql_real_escape_string($f_cod_usua)."' AND p.cod_ppst = h.cod_ppst AND h.cod_usua = u.cod_usua
							UNION
								SELECT '".mysql_real_escape_string($cod_ppst)."' as cod_ppst, 'Sessão de chat' as obs_hist, '4' as tipo_hist, '1' as level_usua, '".mysql_real_escape_string($f_nome)."' as nome_usua, cod_chat,
									(SELECT MIN(dt_chtm) FROM chatmensagens WHERE cod_chat = chat.cod_chat) as data
								FROM chatsessoes as chat
								WHERE cod_usua = '".mysql_real_escape_string($f_cod_usua)."'
							ORDER BY data DESC";
		$db->query();
...
<form name="proposta" id="proposta" method="post" action="<?=$php_self;?>">
	...
	<input type="hidden" name="sit_antiga" id="sit_antiga" value="<?=$f_situacao;?>">

	<?
    ### Alteração de Status ##################################################################### 
    /*
    if($f_situacao>=3 && $f_situacao<=11) {
  ?>
  <a name="novaacao"></a>
  <br><b>Alteração de Situação</b>
	<div class="quadroInterno">
		<div><img src="images/layout/subquadro_t.gif" alt=" " /></div>
		<div class="quadroInternoMeio">
			<table cellpadding=0 cellspacing=5 border=0>
		<tr>
          <td align="right" valign="top">Situação Atual:</td>
          <td align="left"  valign="top"><?=$_SESSION["prop_status"][$f_situacao]?></td>
        </tr>          
        <tr>
          <td align="right" valign="top">Nova Situação:</td>
          <td align="left"  valign="top">
          	<select name="sit_nova" id="sit_nova">
          		<?
          		foreach ($_SESSION["prop_status"] as $key=>$value) {
          			if($key>=3 && $key<=11 && $key!=$f_situacao) {
        ...
        <tr>
          ...
          <td align="left"  valign="top"><img name="btAddEvnt" id="btAddEvnt" src="images/buttons/bt_alterar.gif" value="Alterar Status" class="im" onClick="return addEvento('<?=$crypt->encrypt('status');?>');" /></td>
        </tr>
      </table>
		</div>
		<div><img src="images/layout/subquadro_b.gif" alt=" " /></div>
