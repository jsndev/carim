...
<?
	...
	if($_SESSION['CHAT']=='' || $_SESSION['CHAT'] < 0){
		$db->query="SELECT COD_CHAT, COD_ATEN FROM chatsessoes WHERE COD_ATEN is not NULL and FLG_DISP=0 AND DT_ATEN > (now()-".mysql_real_escape_string($iExpireChat).")  ORDER BY COD_CHAT LIMIT 1";
		$db->query();
		if($db->qrcount > 0){
			$_SESSION['CHAT'] = $db->qrdata[0]["COD_CHAT"];
			$_SESSION['ATEN'] = $db->qrdata[0]["COD_ATEN"];
			$_SESSION['CHAT_ERRO'] = 0;
			$db->query="UPDATE chatsessoes SET FLG_DISP=1, COD_USUA='".mysql_real_escape_string($cLOGIN->iID)."' WHERE COD_CHAT='".mysql_real_escape_string($_SESSION['CHAT'])."'"; 
			$db->query();
			$db->query="
				INSERT INTO chatmensagens ( 
					...
					'".mysql_real_escape_string($cLOGIN->iID)."', 
					NULL, 
					1, 
					0, 
					0, 
			...
			if($_SESSION['CHAT_ERRO'] == -2){
				...
				if ($cLOGIN->iCODPPST) {
					...
					$oHistorico->inserir($cLOGIN->iCODPPST,date("Y-m-d H:i:s"),'Sessão de Chat|'.$_SESSION['CHAT'],'4',$cLOGIN->iID);
				}
	...
	$db->query="SELECT c.*,
								...
								u.NOME_USUA
							FROM chatmensagens c, usuario u
              WHERE c.COD_CHAT='".mysql_real_escape_string($_SESSION['CHAT'])."'
                AND c.FLG_USER = 0
                AND u.COD_USUA=c.COD_ATEN
              ORDER BY c.DT_CHTM";
	$db->query();
	if($db->qrcount > 0){
