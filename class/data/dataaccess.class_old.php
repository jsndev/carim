...
$data=$reg[13];					$data=$header[3];
						...
						// AQUI VOU INSERIR CÓDIGO PARA QUANDO VIER NO ARQUIVO TIPO DE INFORMAÇÃO FN
							...
							$aDadosProponente = $oProponente->pesquisarPorUsuario($iCodUsua);
							$oHistorico = new historico();
							$oHistorico->inserir($aDadosProponente[(count($aDadosProponente)-1)]['cod_ppst'],date("Y-m-d H:i:s"),'Retorno da Previ confirmando Apresentação dos Documentos.','1','');
							...
							$aDadosProponente = $oProponente->pesquisarPorUsuario($iCodUsua);
							$oHistorico = new historico();
							$oHistorico->inserir($aDadosProponente[(count($aDadosProponente)-1)]['cod_ppst'],date("Y-m-d H:i:s"),'Retorno da Previ confirmando Dados de Transferência de Financiamento.','1','');
							...
							$aDadosProponente = $oProponente->pesquisarPorUsuario($iCodUsua);
							$oHistorico = new historico();
							$oHistorico->inserir($aDadosProponente[(count($aDadosProponente)-1)]['cod_ppst'],date("Y-m-d H:i:s"),' Retorno da Previ confirmando Aprovação do Imóvel.','1','');
			...
			foreach ($aMatriculas as $sMatricula) {
				...
				if ($aDadosMatricula[$sMatricula]) {
					if ($aDadosMatricula[$sMatricula][0]['status_pp']!='I'){
											...
											if (@count($aDadosProposta) > 0) {
												foreach ($aDadosProposta as $aProposta) {
													if ($aProposta['flgpropostaativa_ppst'] == '1') {
														$retorno = "Código Identificação ".$sMatricula." possui proposta ativa.<br />";
													}
												}
											}
										}
						...
						$retorno = "Código Identificação ".$sMatricula." Expirado.<br />";
						}
					}else{
						$retorno = "Participante ".$sMatricula." Impedido.<br />";
					}
				} else {
					$retorno = "Código Identificação ".$sMatricula." Inválido.<br />";
				}
			}
