...
$data=$reg[13];					$data=$header[3];
							...
							$this->query = "
								UPDATE listadenomes SET 
									...
									parcaprovada = ".$this->fieldCurrency(mysql_real_escape_string($reg[6])).",
									przaprovado = ".$this->fieldNumber(mysql_real_escape_string($reg[7])).",
									vlsinalaprovado = ".$this->fieldNumber(mysql_real_escape_string($reg[8])).",
									status_pp = '".mysql_real_escape_string($reg[9])."'
								WHERE
									id_lstn = '".mysql_real_escape_string($reg[1])."'
							";
							...
							if ($aDadosProponente[(count($aDadosProponente)-1)]["cod_ppst"]) {
								...
								$oHistorico = new historico();
								$oHistorico->inserir($aDadosProponente[(count($aDadosProponente)-1)]['cod_ppst'],date("Y-m-d H:i:s"),'Retorno da Previ com proposta aprovada (Agencia, Conta Corrente e Renda Informados).','1','');
							}
...
*/		
	...
	function listarAtendentes() {
		...
		$this->query="SELECT * FROM usuario WHERE LEVEL_USUA = 2 ORDER BY NOME_USUA;";
		$this->query();
...
class proposta extends database {
	...
	function saveProposta() {
		...
		if ($sAcao == "concluir" && $aDadosProposta['situacao_ppst'] == "1" && $cLOGIN->iLEVEL_USUA == TPUSER_PROPONENTE) {
			...
			$oHistorico->inserir($_POST['frm_cod_ppst'],date("Y-m-d H:i:s"),'Aprovação de Proposta Cancelada.','1',$cLOGIN->iID);
			$query = "Update proposta set situacao_ppst='5', indcancelamento_ppst='PR' where cod_ppst='".$_POST['frm_cod_ppst']."'";
			$result =mysql_query($query);
			unset($sAcao);			
