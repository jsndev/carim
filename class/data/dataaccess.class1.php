...
$data=$reg[13];					$data=$header[3];
								...
								$mes=substr($data,2,2);
								$ano=substr($data,4,4);
								$dtconvocacao=($ano."-".$mes."-".$dia);							$this->query = "
							...
							$this->query();
							$oHistorico = new historico();
							$oHistorico->inserir($aDadosProponente[(count($aDadosProponente)-1)]['cod_ppst'],date("Y-m-d H:i:s"),'Retorno da Previ com valores de financiamento aprovados.','1','');
							if ($this->qrcount) {
								$this->setProcLines($this->getProcLines()+1);
							}
							...
							$iCodPpst = $this->qrdata[0]["cod_ppst"];
								...
								$oHistorico = new historico();
								$oHistorico->inserir($aDadosProponente[(count($aDadosProponente)-1)]['cod_ppst'],date("Y-m-d H:i:s"),'Retorno da Previ com Fundo de Garantia Liberado (data de assinatura agendada).','1','');

								$oProposta = new proposta();
								$oProposta->setPropostaStatus($aDadosProponente[(count($aDadosProponente)-1)]["cod_ppst"],"9");
							...
							$aDadosProponente = $oProponente->pesquisarPorUsuario($iCodUsua);
							$oHistorico = new historico();
							$oHistorico->inserir($aDadosProponente[(count($aDadosProponente)-1)]['cod_ppst'],date("Y-m-d H:i:s"),'Retorno da Previ com Fundo de Garantia Negado. Erro: '.$reg[14].' - '.$msg_erro.'','1','');


						// AQUI VOU INSERIR CÓDIGO PARA QUANDO VIER NO ARQUIVO TIPO DE INFORMAÇÃO FN
								$this->query = "
									INSERT INTO 
						...
						// AQUI VOU INSERIR CÓDIGO PARA QUANDO VIER NO ARQUIVO TIPO DE INFORMAÇÃO FN
							...
							$oHistorico->inserir($aDadosProponente[(count($aDadosProponente)-1)]['cod_ppst'],date("Y-m-d H:i:s"),'Retorno da Previ com registro do imóvel confirmado.','1','');
							...
							$aDadosProponente = $oProponente->pesquisarPorUsuario($iCodUsua);
							$oHistorico = new historico();
							$oHistorico->inserir($aDadosProponente[(count($aDadosProponente)-1)]['cod_ppst'],date("Y-m-d H:i:s"),'Retorno da Previ com data de assinatura confirmada(Seguros Informados).','1','');
								$data=$reg[13];
								$dia=substr($data,0,2);
								$mes=substr($data,2,2);
								$ano=substr($data,4,4);
							...
							$aDadosProponente = $oProponente->pesquisarPorUsuario($iCodUsua);
							$oHistorico = new historico();
							$oHistorico->inserir($aDadosProponente[(count($aDadosProponente)-1)]['cod_ppst'],date("Y-m-d H:i:s"),'Retorno da previ com Participante Desimpedido.','1','');
						// AQUI VOU INSERIR CÓDIGO PARA QUANDO VIER NO ARQUIVO TIPO DE INFORMAÇÃO FN
								$this->query = "
									UPDATE
										listadenomes
							...
							$aDadosProponente = $oProponente->pesquisarPorUsuario($iCodUsua);
							$oHistorico = new historico();
							$oHistorico->inserir($aDadosProponente[(count($aDadosProponente)-1)]['cod_ppst'],date("Y-m-d H:i:s"),'Retorno da Previ com Participante Impedido.','1','');
						// AQUI VOU INSERIR CÓDIGO PARA QUANDO VIER NO ARQUIVO TIPO DE INFORMAÇÃO FN
								$this->query = "
									UPDATE
										listadenomes
...
class historico extends database {
	...
	function inserir($cod_ppst,$dt_hist,$obs_hist,$tipo_hist,$cod_usua) {
		$this->query = "
			INSERT INTO historico ( cod_ppst,dt_hist,obs_hist,tipo_hist,cod_usua ) VALUES (
				".(!$cod_ppst ? "NULL" : "'".mysql_real_escape_string($cod_ppst)."'").",".(!$dt_hist ? "NULL" : "'".mysql_real_escape_string($dt_hist)."'").",".(!$obs_hist ? "NULL" : "'".mysql_real_escape_string($obs_hist)."'").",".(!$tipo_hist ? "NULL" : "'".mysql_real_escape_string($tipo_hist)."'").",".(!$cod_usua ? "NULL" : "'".mysql_real_escape_string($cod_usua)."'")."
			)
		";
		$this->query();
		return $this->qrdata;
...
class imovel extends database {
	...
	function listar() {
		$this->query = "
			SELECT cod_ppst,tipo_imov,flgaprovacao_imov,area_imov,tpconstrucao_imov,tpcondominio_imov,qtsala_imov,qtquarto_imov,qtbanh_imov,qtgarag_imov,qtpavim_imov,qtdepemp_imov,estconserv_imov,estconspred_imov,nomecartrgi_imov,nrmatrgi_imov,nrlivrgi_imov,nrfolhrgi_imov,nrrgcompvend_imov,nrrggar_imov,endereco_imov,nrendereco_imov,cpendereco_imov,cep_imov,tpimposto_imov,vlavaliacao_imov,dtavaliacao_imov,dtaprovacao_imov,cod_logr,cod_bairro,cod_uf,cod_municipio,tpmoradia_imov,terreo_imov,tmbdspcndop_imov,incomb_imov,ruralfav_imov,emconstr_imov,aquispaimae_imov,possuiirmaos_imov,andar_imov,pavimento_imov,tpapto_imov,flgbloco_imov,numbloco_imov,edificio_imov,conjunto_imov,areautil_imov,areatotal_imov,vagasapto_imov,isolado_imov,nomecondominio_imov,despachante_imov,flgcondominio_imov
			FROM imovel
		";
		$this->query();
		return $this->qrdata;
	...
	function atualizarRegistro($cod_ppst,$nomecartrgi_imov,$nrlivrgi_imov,$nrfolhrgi_imov,$nrrgcompvend_imov,$nrrggar_imov,$dtokregistro_ppst) {
		$this->query = "
			UPDATE imovel SET 
...
class proponente extends database {
	...
	function listar() {
		$this->query = "
			SELECT cpf_ppnt,dtnascimento_ppnt,endereco_ppnt,nrendereco_ppnt,cpendereco_ppnt,cep_ppnt,cod_proponente,cod_logr,cod_bairro,cod_uf,cod_municipio,cod_estciv,telefone_ppnt,cod_ppst,vlfinsol_ppnt,przfinsol_ppnt,vlsinal_ppnt,vlentrada_ppnt,vlprestsol_ppnt,vlcompra_ppnt,vlfinaprov_ppnt,vlprestaprov_ppnt,przaprov_ppnt,despachante_ppnt,flgdevsol_ppnt,email_ppnt
			FROM proponente
		";
		$this->query();
		return $this->qrdata;
	...
	function listarPorUsuarios($aListaUsuarios) {
		if (is_array($aListaUsuarios) && @count($aListaUsuarios) > 0) {
			...
			$this->query();
			//echo $this->query;
			//query funcionando e retornando resultados válidos, mas o qrdata está em branco
			return $this->qrdata;
		} else {
			return false;
		}
	...
	function pesquisarPorCpf($cpf) {
		...
		$this->query = "
			SELECT cpf_ppnt,dtnascimento_ppnt,endereco_ppnt,nrendereco_ppnt,cpendereco_ppnt,cep_ppnt,cod_proponente,cod_logr,cod_bairro,cod_uf,cod_municipio,cod_estciv,telefone_ppnt,cod_ppst,vlfinsol_ppnt,przfinsol_ppnt,vlsinal_ppnt,vlentrada_ppnt,vlprestsol_ppnt,vlcompra_ppnt,vlfinaprov_ppnt,vlprestaprov_ppnt,przaprov_ppnt,despachante_ppnt,flgdevsol_ppnt
			FROM proponente
			WHERE
				cpf_ppnt = '".$cpf."'
		";
	...
	function atualizarPk($cpf_ppnt,$dtnascimento_ppnt,$endereco_ppnt,$nrendereco_ppnt,$cpendereco_ppnt,$cep_ppnt,$cod_proponente,$cod_logr,$cod_bairro,$cod_uf,$cod_municipio,$cod_estciv,$telefone_ppnt,$cod_ppst,$vlfinsol_ppnt,$przfinsol_ppnt,$vlsinal_ppnt,$vlentrada_ppnt,$vlprestsol_ppnt,$vlcompra_ppnt,$vlfinaprov_ppnt,$vlprestaprov_ppnt,$przaprov_ppnt,$despachante_ppnt,$flgdevsol_ppnt = "",$email_ppnt) {
		$this->query = "
			UPDATE proponente SET 
			cpf_ppnt = ".(!$cpf_ppnt ? "NULL" : "'".mysql_real_escape_string($cpf_ppnt)."'").",dtnascimento_ppnt = ".(!$dtnascimento_ppnt ? "NULL" : "'".mysql_real_escape_string($dtnascimento_ppnt)."'").",endereco_ppnt = ".(!$endereco_ppnt ? "NULL" : "'".mysql_real_escape_string($endereco_ppnt)."'").",nrendereco_ppnt = ".(!$nrendereco_ppnt ? "NULL" : "'".mysql_real_escape_string($nrendereco_ppnt)."'").",cpendereco_ppnt = ".(!$cpendereco_ppnt ? "NULL" : "'".mysql_real_escape_string($cpendereco_ppnt)."'").",cep_ppnt = ".(!$cep_ppnt ? "NULL" : "'".mysql_real_escape_string($cep_ppnt)."'").",cod_proponente = ".(!$cod_proponente ? "NULL" : "'".mysql_real_escape_string($cod_proponente)."'").",cod_logr = ".(!$cod_logr ? "NULL" : "'".mysql_real_escape_string($cod_logr)."'").",cod_bairro = ".(!$cod_bairro ? "NULL" : "'".mysql_real_escape_string($cod_bairro)."'").",cod_uf = ".(!$cod_uf ? "NULL" : "'".mysql_real_escape_string($cod_uf)."'").",cod_municipio = ".(!$cod_municipio ? "NULL" : "'".mysql_real_escape_string($cod_municipio)."'").",cod_estciv = ".(!$cod_estciv ? "NULL" : "'".mysql_real_escape_string($cod_estciv)."'").",telefone_ppnt = ".(!$telefone_ppnt ? "NULL" : "'".mysql_real_escape_string($telefone_ppnt)."'").",cod_ppst = ".(!$cod_ppst ? "NULL" : "'".mysql_real_escape_string($cod_ppst)."'").",vlfinsol_ppnt = ".(!$vlfinsol_ppnt ? "NULL" : "'".mysql_real_escape_string($vlfinsol_ppnt)."'").",przfinsol_ppnt = ".(!$przfinsol_ppnt ? "NULL" : "'".mysql_real_escape_string($przfinsol_ppnt)."'").",vlsinal_ppnt = ".(!$vlsinal_ppnt ? "NULL" : "'".mysql_real_escape_string($vlsinal_ppnt)."'").",vlentrada_ppnt = ".(!$vlentrada_ppnt ? "NULL" : "'".mysql_real_escape_string($vlentrada_ppnt)."'").",vlprestsol_ppnt = ".(!$vlprestsol_ppnt ? "NULL" : "'".mysql_real_escape_string($vlprestsol_ppnt)."'").",vlcompra_ppnt = ".(!$vlcompra_ppnt ? "NULL" : "'".mysql_real_escape_string($vlcompra_ppnt)."'").",vlfinaprov_ppnt = ".(!$vlfinaprov_ppnt ? "NULL" : "'".mysql_real_escape_string($vlfinaprov_ppnt)."'").",vlprestaprov_ppnt = ".(!$vlprestaprov_ppnt ? "NULL" : "'".mysql_real_escape_string($vlprestaprov_ppnt)."'").",przaprov_ppnt = ".(!$przaprov_ppnt ? "NULL" : "'".mysql_real_escape_string($przaprov_ppnt)."'").",despachante_ppnt = ".(!$despachante_ppnt ? "NULL" : "'".mysql_real_escape_string($despachante_ppnt)."'").",flgdevsol_ppnt = ".(!$flgdevsol_ppnt ? "NULL" : "'".mysql_real_escape_string($flgdevsol_ppnt)."',email_ppnt = ".(!$email_ppnt ? "NULL" : "'".mysql_real_escape_string($email_ppnt)."'")."")."
			WHERE cod_proponente = '".mysql_real_escape_string($cod_proponente)."' and cod_ppst = '".mysql_real_escape_string($cod_ppst)."'
		";
		$this->query();
		return $this->qrdata;
	...
	function inserir($cpf_ppnt,$dtnascimento_ppnt,$endereco_ppnt,$nrendereco_ppnt,$cpendereco_ppnt,$cep_ppnt,$cod_proponente,$cod_logr,$cod_bairro,$cod_uf,$cod_municipio,$cod_estciv,$telefone_ppnt,$cod_ppst,$vlfinsol_ppnt,$przfinsol_ppnt,$vlsinal_ppnt,$vlentrada_ppnt,$vlprestsol_ppnt,$vlcompra_ppnt,$vlfinaprov_ppnt,$vlprestaprov_ppnt,$przaprov_ppnt,$despachante_ppnt = "",$flgdevsol_ppnt = "",$email_ppnt) {
		$this->query = "
			INSERT INTO proponente ( cpf_ppnt,dtnascimento_ppnt,endereco_ppnt,nrendereco_ppnt,cpendereco_ppnt,cep_ppnt,cod_proponente,cod_logr,cod_bairro,cod_uf,cod_municipio,cod_estciv,telefone_ppnt,email_ppnt,cod_ppst,vlfinsol_ppnt,przfinsol_ppnt,vlsinal_ppnt,vlentrada_ppnt,vlprestsol_ppnt,vlcompra_ppnt,vlfinaprov_ppnt,vlprestaprov_ppnt,przaprov_ppnt,despachante_ppnt,flgdevsol_ppnt ) VALUES (
				".(!$cpf_ppnt ? "NULL" : "'".mysql_real_escape_string($cpf_ppnt)."'").",".(!$dtnascimento_ppnt ? "NULL" : "'".mysql_real_escape_string($dtnascimento_ppnt)."'").",".(!$endereco_ppnt ? "NULL" : "'".mysql_real_escape_string($endereco_ppnt)."'").",".(!$nrendereco_ppnt ? "NULL" : "'".mysql_real_escape_string($nrendereco_ppnt)."'").",".(!$cpendereco_ppnt ? "NULL" : "'".mysql_real_escape_string($cpendereco_ppnt)."'").",".(!$cep_ppnt ? "NULL" : "'".mysql_real_escape_string($cep_ppnt)."'").",".(!$cod_proponente ? "NULL" : "'".mysql_real_escape_string($cod_proponente)."'").",".(!$cod_logr ? "NULL" : "'".mysql_real_escape_string($cod_logr)."'").",".(!$cod_bairro ? "NULL" : "'".mysql_real_escape_string($cod_bairro)."'").",".(!$cod_uf ? "NULL" : "'".mysql_real_escape_string($cod_uf)."'").",".(!$cod_municipio ? "NULL" : "'".mysql_real_escape_string($cod_municipio)."'").",".(!$cod_estciv ? "NULL" : "'".mysql_real_escape_string($cod_estciv)."'").",".(!$telefone_ppnt ? "NULL" : "'".mysql_real_escape_string($telefone_ppnt)."'").",".(!$email_ppnt ? "NULL" : "'".mysql_real_escape_string($email_ppnt)."'").",".(!$cod_ppst ? "NULL" : "'".mysql_real_escape_string($cod_ppst)."'").",".(!$vlfinsol_ppnt ? "NULL" : "'".mysql_real_escape_string($vlfinsol_ppnt)."'").",".(!$przfinsol_ppnt ? "NULL" : "'".mysql_real_escape_string($przfinsol_ppnt)."'").",".(!$vlsinal_ppnt ? "NULL" : "'".mysql_real_escape_string($vlsinal_ppnt)."'").",".(!$vlentrada_ppnt ? "NULL" : "'".mysql_real_escape_string($vlentrada_ppnt)."'").",".(!$vlprestsol_ppnt ? "NULL" : "'".mysql_real_escape_string($vlprestsol_ppnt)."'").",".(!$vlcompra_ppnt ? "NULL" : "'".mysql_real_escape_string($vlcompra_ppnt)."'").",".(!$vlfinaprov_ppnt ? "NULL" : "'".mysql_real_escape_string($vlfinaprov_ppnt)."'").",".(!$vlprestaprov_ppnt ? "NULL" : "'".mysql_real_escape_string($vlprestaprov_ppnt)."'").",".(!$przaprov_ppnt ? "NULL" : "'".mysql_real_escape_string($przaprov_ppnt)."'").",".(!$despachante_ppnt ? "NULL" : "'".mysql_real_escape_string($flgdevsol_ppnt)."'").",".(!$flgdevsol_ppnt ? "NULL" : "'".mysql_real_escape_string($flgdevsol_ppnt)."'")."
			)
		";
		$this->query();
		return $this->qrdata;
...
class proponenteconjuge extends database {
	...
	function listar() {
		$this->query = "
			SELECT cod_proponente,regimebens_ppcj,dtcasamento_ppcj,nome_ppcj,cod_pais,nrrg_ppcj,orgrg_ppcj,dtrg_ppcj,cpf_pccj,flgtrabalha_ppcj,empresa_ppcj,dtadmissaoemp_ppcj,enderecoemp_ppcj,numeroemp_ppcj,complementoemp_ppcj,bairroemp_ppcj,cidadeemp_ppcj,estadoemp_ppcj,telefoneemp_ppcj,cargoemp_ppcj,salarioemp_ppcj,cod_ppst,despachante_ppcj
...
class proponenteconjugepacto extends database {
	...
	function listar() {
		$this->query = "
			SELECT cod_proponente,data_pcpa,locallavracao_pcpa,livro_pcpa,folha_pcpa,numeroregistro_pcpa,cod_ppst
	...
	function atualizarPk($cod_proponente,$data_pcpa,$locallavracao_pcpa,$livro_pcpa,$folha_pcpa,$numeroregistro_pcpa,$cod_ppst) {
		$this->query = "
			UPDATE proponenteconjugepacto SET 
			cod_proponente = ".(!$cod_proponente ? "NULL" : "'".mysql_real_escape_string($cod_proponente)."'").",data_pcpa = ".(!$data_pcpa ? "NULL" : "'".mysql_real_escape_string($data_pcpa)."'").",locallavracao_pcpa = ".(!$locallavracao_pcpa ? "NULL" : "'".mysql_real_escape_string($locallavracao_pcpa)."'").",livro_pcpa = ".(!$livro_pcpa ? "NULL" : "'".mysql_real_escape_string($livro_pcpa)."'").",folha_pcpa = ".(!$folha_pcpa ? "NULL" : "'".mysql_real_escape_string($folha_pcpa)."'").",numeroregistro_pcpa = ".(!$numeroregistro_pcpa ? "NULL" : "'".mysql_real_escape_string($numeroregistro_pcpa)."'").",cod_ppst = ".(!$cod_ppst ? "NULL" : "'".mysql_real_escape_string($cod_ppst)."'")."
			WHERE cod_proponente = '".mysql_real_escape_string($cod_proponente)."' and cod_ppst = '".mysql_real_escape_string($cod_ppst)."'
		";
		$this->query();
		return $this->qrdata;
	...
	function inserir($cod_proponente,$data_pcpa,$locallavracao_pcpa,$livro_pcpa,$folha_pcpa,$numeroregistro_pcpa,$cod_ppst) {
		$this->query = "
			INSERT INTO proponenteconjugepacto ( cod_proponente,data_pcpa,locallavracao_pcpa,livro_pcpa,folha_pcpa,numeroregistro_pcpa,cod_ppst ) VALUES (
				".(!$cod_proponente ? "NULL" : "'".mysql_real_escape_string($cod_proponente)."'").",".(!$data_pcpa ? "NULL" : "'".mysql_real_escape_string($data_pcpa)."'").",".(!$locallavracao_pcpa ? "NULL" : "'".mysql_real_escape_string($locallavracao_pcpa)."'").",".(!$livro_pcpa ? "NULL" : "'".mysql_real_escape_string($livro_pcpa)."'").",".(!$folha_pcpa ? "NULL" : "'".mysql_real_escape_string($folha_pcpa)."'").",".(!$numeroregistro_pcpa ? "NULL" : "'".mysql_real_escape_string($numeroregistro_pcpa)."'").",".(!$cod_ppst ? "NULL" : "'".mysql_real_escape_string($cod_ppst)."'")."
			)
		";
		$this->query();
		return $this->qrdata;
...
class proposta extends database {
	...
	function pesquisarPk($cod_ppst) {
		$this->query = "
			SELECT cod_ppst,data_ppst,situacao_ppst,dtapresdoc_ppst,dtiniexigencia_ppst,dtfimexigencia_ppst,dtaprovacao_ppst,dtasscontrato_ppst,dtokregistro_ppst,vlfinsol_ppst,przfinsol_ppst,valordevsinalsol_ppst,pricesac_ppst,valorcompra_ppst,valorfgts_ppst,taxajuros_ppst,valorseguro_ppst,valormanutencao_ppst,vlprestsol_ppst,indcancelamento_ppst,valorboletoaval_ppst,flgformapagto_ppst,flgboletoavalpago_ppst,dtagend_asscontr_ppst,flgrespostavalor_ppst,dtpagtoboleto_ppst,flgaprovacaoprevi_ppst,dtremessacontrato_ppst,flgpropostaativa_ppst,flgaprovimovel_ppst
			FROM proposta
			WHERE cod_ppst = '".mysql_real_escape_string($cod_ppst)."'
		";
		//echo $this->query;
	...
	function inserir($data_ppst,$situacao_ppst,$dtapresdoc_ppst,$dtiniexigencia_ppst,$dtfimexigencia_ppst,$dtaprovacao_ppst,$dtasscontrato_ppst,$dtokregistro_ppst,$vlfinsol_ppst,$przfinsol_ppst,$valordevsinalsol_ppst,$pricesac_ppst,$valorcompra_ppst,$valorfgts_ppst,$taxajuros_ppst,$valorseguro_ppst,$valormanutencao_ppst,$vlprestsol_ppst,$indcancelamento_ppst,$valorboletoaval_ppst,$flgboletoavalpago_ppst,$dtagend_asscontr_ppst,$flgrespostavalor_ppst,$dtpagtoboleto_ppst,$flgaprovacaoprevi_ppst,$dtremessacontrato_ppst,$flgpropostaativa_ppst) {
		$this->query = "
			INSERT INTO proposta ( data_ppst,situacao_ppst,dtapresdoc_ppst,dtiniexigencia_ppst,dtfimexigencia_ppst,dtaprovacao_ppst,dtasscontrato_ppst,dtokregistro_ppst,vlfinsol_ppst,przfinsol_ppst,valordevsinalsol_ppst,pricesac_ppst,valorcompra_ppst,valorfgts_ppst,taxajuros_ppst,valorseguro_ppst,valormanutencao_ppst,vlprestsol_ppst,indcancelamento_ppst,valorboletoaval_ppst,flgboletoavalpago_ppst,dtagend_asscontr_ppst,flgrespostavalor_ppst,dtpagtoboleto_ppst,flgaprovacaoprevi_ppst,dtremessacontrato_ppst,flgpropostaativa_ppst ) VALUES (
	...
	function listarPorStatus($aStatus, $limite = false) {
		$sComplementoQuery = '';
		global $cLOGIN;
		
		if ($_POST['filtro_nome']) {
			//utiliza a classe usuário para pesquisar se tem nome
			...
			if (is_array($aUsuarios) && @count($aUsuarios) > 0) {
				...
				//$listaUsuas é uma array com as chaves da procura, mas o problema parece estar em seu retorno
		...
		$this->query = "
			...
			FROM proposta
			WHERE situacao_ppst IN ('".implode("','",$aStatus)."') ".$complementoFiltroNome." ".$complementoFiltroMatricula." ".$complementoFiltroCpf." ".$complementoFiltroDespachante. "
		";
		$this->query();
	...
	function geraBarraProposta() {
		...
		if($ant_pagina > 0)
			echo "<a href='lista_propostas.php?pagina=" . $ant_pagina . "'> << </a> ";	
		for($i=1; $i<=$paginas; $i++){
			
			if($i != $pagina_atual)
				echo "<a href='lista_propostas.php?pagina=" . $i . "'>" . $i . "</a> "; 
			else
				echo " $i "; 			
			
		}
		if($prox_pagina <= $paginas)
			echo "<a href='lista_propostas.php?pagina=" . $prox_pagina . "'> >> </a> ";
	...
	function getListaProposta($ctAll = false, $limite = false) {
		/* @var $cLOGIN login */
		global $cLOGIN,$aPROPOSTALISTA;
		if ($ctAll) {
		...
		if ($aParams) {
			$aDadosProposta = $this->listarPorStatus($aParams, $limite);
	...
	function incluirProposta($aDados) {
		...
		if (FLG_PREVI == true) {
			...
			foreach ($aMatriculas as $sMatricula) {
				...
				if ($aDadosMatricula[$sMatricula]) {
					if ($aDadosMatricula[$sMatricula][0]['status_pp']!='I'){
											...
											if (@count($aDadosProposta) > 0) {
												foreach ($aDadosProposta as $aProposta) {
													if ($aProposta['flgpropostaativa_ppst'] == '1') {
														$retorno = "Matrícula ".$sMatricula." possui proposta ativa.<br />";
													}
												}
											}
										}
						...
						$retorno = "Matrícula ".$sMatricula." Expirada.<br />";
						}
					}else{
						$retorno = "Matrícula ".$sMatricula." Impedida.<br />";
					}
				} else {
					$retorno = "Matrícula ".$sMatricula." inválida.<br />";
				}
			}
			...
			if ($retorno === true) {
				...
				$this->inserir(date("Y-m-d H:i:s"),"1","","","","","","","","","","","","","","","","","","","","","","","","","1");
				$cod_ppst = $this->insertId;
				if ($cod_ppst) {
					$sSenhaUsuario = md5($aDados["cad_senha"]);
					foreach ($aMatriculas as $idxMatricula => $sMatricula) {
						...
						if (!$cod_usua) {
							if ($this->getErrNo() == DB_ERR_UNIQUE) {
								$retorno = 'Endereço de e-mail '.$sEmailUsuario.' em uso por outro usuário.';
							} else {
								$retorno = 'Erro com dados de usuários.';
							}
						} else {
	...
	function getPropostaDuplicidadeRGCPFCNPJ($aLista,$sProcura,$tipo = "CPF") {
		if (array_search($sProcura,$aLista) !== false) {
			if ($tipo == "CPF") {
				return "CPF ".utils::formataCPF($sProcura)." está em uso nesta proposta.";
			} elseif ($tipo == "RG") {
				return "RG ".utils::formataRG($sProcura)." está em uso nesta proposta.";
			} elseif ($tipo == "CNPJ") {
				return "CNPJ ".utils::formataCnpj($sProcura)." está em uso nesta proposta.";
			}
		}
		return true;
	}
			...
			foreach ($aRetorno['proponentes'] as $idxProponente => $aProponente) {
				...
				if ($aRetorno['proponentes'][$idxProponente]['cpf_ppnt'] != '') {
					if (!proposta::checkPropostaDuplicidadeCPFProponente($cod_ppst,$aRetorno['proponentes'][$idxProponente]['cpf_ppnt'])) {
						$aRetorno['proponentes'][$idxProponente]['erroValidacao'] = "CPF ".utils::formataCPF($aRetorno['proponentes'][$idxProponente]['cpf_ppnt'])." está em uso por outra proposta ativa.";
					} else {
	...
	function saveProposta() {
		...
		if ($sAcao == "concluir" && $aDadosProposta['situacao_ppst'] == "1" && $cLOGIN->iLEVEL_USUA == TPUSER_PROPONENTE) {
			$oHistorico->inserir($_POST['frm_cod_ppst'],date("Y-m-d H:i:s"),'Proposta enviada à Athos','1',$cLOGIN->iID);
			...
			$oHistorico->inserir($_POST['frm_cod_ppst'],date("Y-m-d H:i:s"),'Proposta concluída e enviada ao advogado','1',$cLOGIN->iID);
			...
			$query = "Update proposta set indcancelamento_ppst= NULL, situacao_ppst='7' where cod_ppst='".$_POST['frm_cod_ppst']."'";
			$result =mysql_query($query);
			...
			$this->setPropostaStatus($cod_ppst,"5");
			unset($sAcao);
		} elseif ($sAcao == "cancelar_aprovacao_proposta" && $aDadosProposta['situacao_ppst'] == "7" && $cLOGIN->iLEVEL_USUA == TPUSER_JURIDICO) {
			$oHistorico->inserir($_POST['frm_cod_ppst'],date("Y-m-d H:i:s"),'Aprovação de Proposta Cancelada.','1',$cLOGIN->iID);
			$query = "Update proposta set situacao_ppst='5', indcancelamento_ppst='AP' where cod_ppst='".$_POST['frm_cod_ppst']."'";
			$result =mysql_query($query);
			unset($sAcao);
		} elseif ($sAcao == "cancelar_aprovacao_proposta_fgts" && $aDadosProposta['situacao_ppst'] == "7" && $cLOGIN->iLEVEL_USUA == TPUSER_JURIDICO) {
			$oHistorico->inserir($_POST['frm_cod_ppst'],date("Y-m-d H:i:s"),'Aprovação de Proposta Cancelada.','1',$cLOGIN->iID);
			$query = "Update proposta set situacao_ppst='5', dtaprovacao_ppst= NULL where cod_ppst='".$_POST['frm_cod_ppst']."'";
			$result =mysql_query($query);
			unset($sAcao);			
			...
			$oHistorico->inserir($_POST['frm_cod_ppst'],date("Y-m-d H:i:s"),'Informados os dados de registro do imóvel.','1',$cLOGIN->iID);
		...
		if (($sAcao == "addVend" || $sAcao == "salvar" || $sAcao == "savePpnt" || $sAcao == "saveVend" || $sAcao == "saveSocio") && $cod_ppst) {
			...
			$Nome_ppnt=$_POST['nome_ppnt'];
			$Email_ppnt=$_POST['email_usua'];
			
			$query="Update usuario set nome_usua='".$Nome_ppnt."', email_usua='".$Email_ppnt."' where cod_usua='".$_POST['frm_cod_ppnt']."'";
			mysql_query($query);
...
//if (!$aDadosProposta['imovel']['dtaprovacao_imov'] && ($aDadosProposta['situacao_ppst'] == "1" || $aDadosProposta['situacao_ppst'] == "3" || $aDadosProposta['situacao_ppst'] == "5")) {
			...
			if ($_POST['frm_cod_ppnt'] != '' && ($aDadosProposta['situacao_ppst'] == "1" || ($aDadosProposta['situacao_ppst'] == "3" && $cLOGIN->iLEVEL_USUA == TPUSER_ATENDENTE) || $aDadosProposta['situacao_ppst'] == "5" ) ) {
				...
				$aDadosProponenteConjuge[0]['nrrg_ppcj'] = utils::limpaNumeros($_POST['nrrg_ppcj']);
				...
				proponenteconjugepacto::inserir($_POST['frm_cod_ppnt'],$aDadosProponenteConjugePacto[0]['data_pcpa'],$aDadosProponenteConjugePacto[0]['locallavracao_pcpa'],$aDadosProponenteConjugePacto[0]['livro_pcpa'],$aDadosProponenteConjugePacto[0]['folha_pcpa'],$aDadosProponenteConjugePacto[0]['numeroregistro_pcpa'],$cod_ppst);
        ...
        $aDadosVendedor[0]['email_vend'] = $_POST['email_vend'];
			...
			if ($cod_vend != "" && ($aDadosProposta['situacao_ppst'] == "1" || ($aDadosProposta['situacao_ppst'] == "3" && $cLOGIN->iLEVEL_USUA == TPUSER_ATENDENTE) || $aDadosProposta['situacao_ppst'] == "5")) {
				/////////////////////$_POST['vend_tipo'];
				...
				if ($_POST['vend_tipo'] != "") {
					...
					if ($_POST['vend_tipo'] == '1') {
						...
						vendfis::inserir($cpf_vfisica,$sexo_vfisica,$dtnascimento_vfisica,$natur_vfisica,$nrrg_vfisica,$dtrg_vfisica,$orgrg_vfisica,$nomeconj_vfisica,$nomepai_vfisica,$nomemae_vfisica,$vlrenda_vfisica,$nrinss_vfisica,$cod_pais,$cod_ppst,$cod_tpdoc,$cod_prof,$cod_estciv,$cod_vend,$despachante_vfisica);
						...
						vendfisconjuge::inserir($cod_vend,$cod_ppst,$regimebens_vfcj,$dtcasamento_vfcj,$nome_vfcj,$cod_pais,$nrrg_vfcj,$orgrg_vfcj,$dtrg_vfcj,$cpf_pccj,$flgtrabalha_vfcj,$empresa_vfcj,$dtadmissaoemp_vfcj,$enderecoemp_vfcj,$numeroemp_vfcj,$complementoemp_vfcj,$bairroemp_vfcj,$cidadeemp_vfcj,$estadoemp_vfcj,$telefoneemp_vfcj,$cargoemp_vfcj,$salarioemp_vfcj,$despachante_vfcj);
						...
						if ($aDadosVendedorJur) {
							vendjur::atualizarPk($cod_ppst,$cnpj_vjur,$isenpis_vjur,$isencofins_vjur,$isencsll_vjur,$cod_cnae,$cod_vend,$despachante_vjur);
						} else {
							vendjur::inserir($cod_ppst,$cnpj_vjur,$isenpis_vjur,$isencofins_vjur,$isencsll_vjur,$cod_cnae,$cod_vend,$despachante_vjur);
						}
						...
						if ($sAcao == "saveSocio") {
							...
							if ($_POST['frm_cod_socio']) {
								vendjursocio::atualizarPk($cod_ppst,$cod_vjsoc,$nome_vjsoc,$nick_vjsoc,$endereco_vjsoc,$nrendereco_vjsoc,$cpendereco_vjsoc,$cep_vjsoc,$telefone_vjsoc,$cpf_vjsoc,$sexo_vjsoc,$cod_pais,$cod_logr,$cod_bairro,$cod_uf,$cod_municipio,$cod_vend);
							} else {
								vendjursocio::inserir($cod_ppst,$nome_vjsoc,$nick_vjsoc,$endereco_vjsoc,$nrendereco_vjsoc,$cpendereco_vjsoc,$cep_vjsoc,$telefone_vjsoc,$cpf_vjsoc,$sexo_vjsoc,$cod_pais,$cod_logr,$cod_bairro,$cod_uf,$cod_municipio,$cod_vend);
							}
						}
					}
				}
			...
			if ($_POST['frm_cod_ppnt'] != '' && ($aDadosProposta['situacao_ppst'] == "1" || ($aDadosProposta['situacao_ppst'] == "3" && ($cLOGIN->iLEVEL_USUA == TPUSER_ATENDENTE || $cLOGIN->iLEVEL_USUA == TPUSER_PROPONENTE)) || $aDadosProposta['situacao_ppst'] == "5" ) ) {
				$aProponente = proponente::pesquisarPk($_POST['frm_cod_ppnt'],$cod_ppst);
				clistproponente::deletarPorPropostaProponente($cod_ppst,$_POST['frm_cod_ppnt']);
				clistproponenteconjuge::deletarPorPropostaProponente($cod_ppst,$_POST['frm_cod_ppnt']);
				clistproponentefgts::deletarPorPropostaProponente($cod_ppst,$_POST['frm_cod_ppnt']);
			...
			$oHistorico->inserir($_POST['frm_cod_ppst'],date("Y-m-d H:i:s"),'Imóvel aprovado.','1',$cLOGIN->iID);
			...
			$aDadosProposta['imovel']['nomecondominio_imov'] = $_POST['nome_condominio_imov'];
			$aDadosProposta['imovel']['despachante_imov'] = $_POST['cod_despachante_imov'];
			imovel::atualizarPk($aDadosProposta['imovel']['cod_ppst'],$aDadosProposta['imovel']['tipo_imov'],$aDadosProposta['imovel']['flgaprovacao_imov'],$aDadosProposta['imovel']['area_imov'],$aDadosProposta['imovel']['tpconstrucao_imov'],$aDadosProposta['imovel']['tpcondominio_imov'],$aDadosProposta['imovel']['qtsala_imov'],$aDadosProposta['imovel']['qtquarto_imov'],$aDadosProposta['imovel']['qtbanh_imov'],$aDadosProposta['imovel']['qtgarag_imov'],$aDadosProposta['imovel']['qtpavim_imov'],$aDadosProposta['imovel']['qtdepemp_imov'],$aDadosProposta['imovel']['estconserv_imov'],$aDadosProposta['imovel']['estconspred_imov'],$aDadosProposta['imovel']['nomecartrgi_imov'],$aDadosProposta['imovel']['nrmatrgi_imov'],$aDadosProposta['imovel']['nrlivrgi_imov'],$aDadosProposta['imovel']['nrfolhrgi_imov'],$aDadosProposta['imovel']['nrrgcompvend_imov'],$aDadosProposta['imovel']['nrrggar_imov'],$aDadosProposta['imovel']['endereco_imov'],$aDadosProposta['imovel']['nrendereco_imov'],$aDadosProposta['imovel']['cpendereco_imov'],$aDadosProposta['imovel']['cep_imov'],$aDadosProposta['imovel']['tpimposto_imov'],$aDadosProposta['imovel']['vlavaliacao_imov'],$aDadosProposta['imovel']['dtavaliacao_imov'],$aDadosProposta['imovel']['dtavaliacao_imov'],$aDadosProposta['imovel']['dtaprovacao_imov'],$aDadosProposta['imovel']['cod_logr'],$aDadosProposta['imovel']['cod_bairro'],$aDadosProposta['imovel']['cod_uf'],$aDadosProposta['imovel']['cod_municipio'],$aDadosProposta['imovel']['tpmoradia_imov'],$aDadosProposta['imovel']['terreo_imov'],$aDadosProposta['imovel']['tmbdspcndop_imov'],$aDadosProposta['imovel']['incomb_imov'],$aDadosProposta['imovel']['ruralfav_imov'],$aDadosProposta['imovel']['emconstr_imov'],$aDadosProposta['imovel']['aquispaimae_imov'],$aDadosProposta['imovel']['possuiirmaos_imov'],$aDadosProposta['imovel']['andar_imov'],$aDadosProposta['imovel']['pavimento_imov'],$aDadosProposta['imovel']['tpapto_imov'],$aDadosProposta['imovel']['flgbloco_imov'],$aDadosProposta['imovel']['numbloco_imov'],$aDadosProposta['imovel']['edificio_imov'],$aDadosProposta['imovel']['conjunto_imov'],$aDadosProposta['imovel']['areautil_imov'],$aDadosProposta['imovel']['areatotal_imov'],$aDadosProposta['imovel']['vagasapto_imov'],$aDadosProposta['imovel']['isolado_imov'],$aDadosProposta['imovel']['nomecondominio_imov'],$aDadosProposta['imovel']['despachante_imov'],$aDadosProposta['imovel']['flgcondominio_imov']);
		...
		$vlentrada=$_POST['vlentrada_ppnt'];
			...
			$oHistorico->inserir($_POST['frm_cod_ppst'],date("Y-m-d H:i:s"),'Aprovação do Imóvel Cancelada.','1',$cLOGIN->iID);
			...
			imovel::atualizarRegistro($_POST['frm_cod_ppst'],$_POST['nomecartrgi_imov'],$_POST['nrlivrgi_imov'],$_POST['nrfolhrgi_imov'],$_POST['nrrgcompvend_imov'],$_POST['nrrggar_imov'],$_POST['dtokregistro_ppst']);
			$oHistorico->inserir($_POST['frm_cod_ppst'],date("Y-m-d H:i:s"),'Registro de Imóvel','1',$cLOGIN->iID);
			...
			$oHistorico->inserir($_POST['frm_cod_ppst'],date("Y-m-d H:i:s"),'Registro de Imóvel Cancelado','1',$cLOGIN->iID);
			//echo $sAcao;
...
class vendedor extends database {
	...
	function listar() {
		$this->query = "
			SELECT cod_ppst,cod_vend,tipo_vend,nome_vend,nick_vend,endereco_vend,nrendereco_vend,cep_vend,telefone_vend,nrcc_vend,dvcc_vend,nrag_vend,cod_bairro,cod_logr,cod_uf,cod_municipio,cpendereco_vend,percentualvenda_vend,despachante_vend,email_vend
			FROM vendedor
		";
		$this->query();
		return $this->qrdata;
	...
	function inserir($cod_ppst,$tipo_vend,$nome_vend,$nick_vend,$endereco_vend,$nrendereco_vend,$cep_vend,$telefone_vend,$nrcc_vend,$dvcc_vend,$nrag_vend,$cod_bairro,$cod_logr,$cod_uf,$cod_municipio,$cpendereco_vend,$percentualvenda_vend,$despachante_vend,$email_vend) {
		$this->query = "
			INSERT INTO vendedor ( cod_ppst,tipo_vend,nome_vend,nick_vend,endereco_vend,nrendereco_vend,cep_vend,telefone_vend,nrcc_vend,dvcc_vend,nrag_vend,cod_bairro,cod_logr,cod_uf,cod_municipio,cpendereco_vend,percentualvenda_vend,despachante_vend,email_vend ) VALUES (
				".(!$cod_ppst ? "NULL" : "'".mysql_real_escape_string($cod_ppst)."'").",".(!$tipo_vend ? "NULL" : "'".mysql_real_escape_string($tipo_vend)."'").",".(!$nome_vend ? "NULL" : "'".mysql_real_escape_string($nome_vend)."'").",".(!$nick_vend ? "NULL" : "'".mysql_real_escape_string($nick_vend)."'").",".(!$endereco_vend ? "NULL" : "'".mysql_real_escape_string($endereco_vend)."'").",".(!$nrendereco_vend ? "NULL" : "'".mysql_real_escape_string($nrendereco_vend)."'").",".(!$cep_vend ? "NULL" : "'".mysql_real_escape_string($cep_vend)."'").",".(!$telefone_vend ? "NULL" : "'".mysql_real_escape_string($telefone_vend)."'").",".(!$nrcc_vend ? "NULL" : "'".mysql_real_escape_string($nrcc_vend)."'").",".(!$dvcc_vend ? "NULL" : "'".mysql_real_escape_string($dvcc_vend)."'").",".(!$nrag_vend ? "NULL" : "'".mysql_real_escape_string($nrag_vend)."'").",".(!$cod_bairro ? "NULL" : "'".mysql_real_escape_string($cod_bairro)."'").",".(!$cod_logr ? "NULL" : "'".mysql_real_escape_string($cod_logr)."'").",".(!$cod_uf ? "NULL" : "'".mysql_real_escape_string($cod_uf)."'").",".(!$cod_municipio ? "NULL" : "'".mysql_real_escape_string($cod_municipio)."'").",".(!$cpendereco_vend ? "NULL" : "'".mysql_real_escape_string($cpendereco_vend)."'").",".(!$percentualvenda_vend ? "NULL" : "'".mysql_real_escape_string($percentualvenda_vend)."'").",".(!$despachante_vend ? "NULL" : "'".mysql_real_escape_string($despachante_vend)."'").",".(!$email_vend ? "NULL" : "'".mysql_real_escape_string($email_vend)."'")."
			)
		";
		$this->query();
		return $this->qrdata;
...
class vendfis extends database {
	...
	function listar() {
		$this->query = "
			SELECT cpf_vfisica,sexo_vfisica,dtnascimento_vfisica,natur_vfisica,nrrg_vfisica,dtrg_vfisica,orgrg_vfisica,nomeconj_vfisica,nomepai_vfisica,nomemae_vfisica,vlrenda_vfisica,nrinss_vfisica,cod_pais,cod_ppst,cod_tpdoc,cod_prof,cod_estciv,cod_vend,despachante_vfisica
			FROM vendfis
		";
		$this->query();
		return $this->qrdata;
...
class vendfisconjuge extends database {
	...
	function listar() {
		$this->query = "
			SELECT cod_vend,cod_ppst,regimebens_vfcj,dtcasamento_vfcj,nome_vfcj,cod_pais,nrrg_vfcj,orgrg_vfcj,dtrg_vfcj,cpf_pccj,flgtrabalha_vfcj,empresa_vfcj,dtadmissaoemp_vfcj,enderecoemp_vfcj,numeroemp_vfcj,complementoemp_vfcj,bairroemp_vfcj,cidadeemp_vfcj,estadoemp_vfcj,telefoneemp_vfcj,cargoemp_vfcj,salarioemp_vfcj,despachante_vfcj
			FROM vendfisconjuge
		";
		$this->query();
		return $this->qrdata;
...
class vendjur extends database {
	...
	function listar() {
		$this->query = "
			SELECT cod_ppst,cnpj_vjur,isenpis_vjur,isencofins_vjur,isencsll_vjur,cod_cnae,cod_vend,despachante_vjur
			FROM vendjur
		";
		$this->query();
		return $this->qrdata;
	...
	function atualizarPk($cod_ppst,$cnpj_vjur,$isenpis_vjur,$isencofins_vjur,$isencsll_vjur,$cod_cnae,$cod_vend,$despachante_vjur) {
		$this->query = "
			UPDATE vendjur SET 
			cod_ppst = ".(!$cod_ppst ? "NULL" : "'".mysql_real_escape_string($cod_ppst)."'").",cnpj_vjur = ".(!$cnpj_vjur ? "NULL" : "'".mysql_real_escape_string($cnpj_vjur)."'").",isenpis_vjur = ".(!$isenpis_vjur ? "NULL" : "'".mysql_real_escape_string($isenpis_vjur)."'").",isencofins_vjur = ".(!$isencofins_vjur ? "NULL" : "'".mysql_real_escape_string($isencofins_vjur)."'").",isencsll_vjur = ".(!$isencsll_vjur ? "NULL" : "'".mysql_real_escape_string($isencsll_vjur)."'").",cod_cnae = ".(!$cod_cnae ? "NULL" : "'".mysql_real_escape_string($cod_cnae)."'").",cod_vend = ".(!$cod_vend ? "NULL" : "'".mysql_real_escape_string($cod_vend)."'").",despachante_vjur = ".(!$despachante_vjur ? "NULL" : "'".mysql_real_escape_string($despachante_vjur)."'")."
			WHERE cod_ppst = '".mysql_real_escape_string($cod_ppst)."' and cod_vend = '".mysql_real_escape_string($cod_vend)."'
		";
		$this->query();
		return $this->qrdata;
	...
	function inserir($cod_ppst,$cnpj_vjur,$isenpis_vjur,$isencofins_vjur,$isencsll_vjur,$cod_cnae,$cod_vend,$despachante_vjur) {
		$this->query = "
			INSERT INTO vendjur ( cod_ppst,cnpj_vjur,isenpis_vjur,isencofins_vjur,isencsll_vjur,cod_cnae,cod_vend,despachante_vjur ) VALUES (
				".(!$cod_ppst ? "NULL" : "'".mysql_real_escape_string($cod_ppst)."'").",".(!$cnpj_vjur ? "NULL" : "'".mysql_real_escape_string($cnpj_vjur)."'").",".(!$isenpis_vjur ? "NULL" : "'".mysql_real_escape_string($isenpis_vjur)."'").",".(!$isencofins_vjur ? "NULL" : "'".mysql_real_escape_string($isencofins_vjur)."'").",".(!$isencsll_vjur ? "NULL" : "'".mysql_real_escape_string($isencsll_vjur)."'").",".(!$cod_cnae ? "NULL" : "'".mysql_real_escape_string($cod_cnae)."'").",".(!$cod_vend ? "NULL" : "'".mysql_real_escape_string($cod_vend)."'").",".(!$despachante_vjur ? "NULL" : "'".mysql_real_escape_string($despachante_vjur)."'")."
			)
		";
		$this->query();
		return $this->qrdata;
...
class vendjursocio extends database {
	...
	function listar() {
		$this->query = "
			SELECT cod_ppst,cod_vjsoc,nome_vjsoc,nick_vjsoc,endereco_vjsoc,nrendereco_vjsoc,cpendereco_vjsoc,cep_vjsoc,telefone_vjsoc,cpf_vjsoc,sexo_vjsoc,cod_pais,cod_logr,cod_bairro,cod_uf,cod_municipio,cod_vend
			FROM vendjursocio
		";
		$this->query();
		return $this->qrdata;
	...
	function atualizarPk($cod_ppst,$cod_vjsoc,$nome_vjsoc,$nick_vjsoc,$endereco_vjsoc,$nrendereco_vjsoc,$cpendereco_vjsoc,$cep_vjsoc,$telefone_vjsoc,$cpf_vjsoc,$sexo_vjsoc,$cod_pais,$cod_logr,$cod_bairro,$cod_uf,$cod_municipio,$cod_vend) {
		$this->query = "
			UPDATE vendjursocio SET 
			cod_ppst = ".(!$cod_ppst ? "NULL" : "'".mysql_real_escape_string($cod_ppst)."'").",nome_vjsoc = ".(!$nome_vjsoc ? "NULL" : "'".mysql_real_escape_string($nome_vjsoc)."'").",nick_vjsoc = ".(!$nick_vjsoc ? "NULL" : "'".mysql_real_escape_string($nick_vjsoc)."'").",endereco_vjsoc = ".(!$endereco_vjsoc ? "NULL" : "'".mysql_real_escape_string($endereco_vjsoc)."'").",nrendereco_vjsoc = ".(!$nrendereco_vjsoc ? "NULL" : "'".mysql_real_escape_string($nrendereco_vjsoc)."'").",cpendereco_vjsoc = ".(!$cpendereco_vjsoc ? "NULL" : "'".mysql_real_escape_string($cpendereco_vjsoc)."'").",cep_vjsoc = ".(!$cep_vjsoc ? "NULL" : "'".mysql_real_escape_string($cep_vjsoc)."'").",telefone_vjsoc = ".(!$telefone_vjsoc ? "NULL" : "'".mysql_real_escape_string($telefone_vjsoc)."'").",cpf_vjsoc = ".(!$cpf_vjsoc ? "NULL" : "'".mysql_real_escape_string($cpf_vjsoc)."'").",sexo_vjsoc = ".(!$sexo_vjsoc ? "NULL" : "'".mysql_real_escape_string($sexo_vjsoc)."'").",cod_pais = ".(!$cod_pais ? "NULL" : "'".mysql_real_escape_string($cod_pais)."'").",cod_logr = ".(!$cod_logr ? "NULL" : "'".mysql_real_escape_string($cod_logr)."'").",cod_bairro = ".(!$cod_bairro ? "NULL" : "'".mysql_real_escape_string($cod_bairro)."'").",cod_uf = ".(!$cod_uf ? "NULL" : "'".mysql_real_escape_string($cod_uf)."'").",cod_municipio = ".(!$cod_municipio ? "NULL" : "'".mysql_real_escape_string($cod_municipio)."'").",cod_vend = ".(!$cod_vend ? "NULL" : "'".mysql_real_escape_string($cod_vend)."'")."
			WHERE cod_vjsoc = '".mysql_real_escape_string($cod_vjsoc)."'
		";
		$this->query();
		return $this->qrdata;
	...
	function inserir($cod_ppst,$nome_vjsoc,$nick_vjsoc,$endereco_vjsoc,$nrendereco_vjsoc,$cpendereco_vjsoc,$cep_vjsoc,$telefone_vjsoc,$cpf_vjsoc,$sexo_vjsoc,$cod_pais,$cod_logr,$cod_bairro,$cod_uf,$cod_municipio,$cod_vend) {
		$this->query = "
			INSERT INTO vendjursocio ( cod_ppst,nome_vjsoc,nick_vjsoc,endereco_vjsoc,nrendereco_vjsoc,cpendereco_vjsoc,cep_vjsoc,telefone_vjsoc,cpf_vjsoc,sexo_vjsoc,cod_pais,cod_logr,cod_bairro,cod_uf,cod_municipio,cod_vend ) VALUES (
				".(!$cod_ppst ? "NULL" : "'".mysql_real_escape_string($cod_ppst)."'").",".(!$nome_vjsoc ? "NULL" : "'".mysql_real_escape_string($nome_vjsoc)."'").",".(!$nick_vjsoc ? "NULL" : "'".mysql_real_escape_string($nick_vjsoc)."'").",".(!$endereco_vjsoc ? "NULL" : "'".mysql_real_escape_string($endereco_vjsoc)."'").",".(!$nrendereco_vjsoc ? "NULL" : "'".mysql_real_escape_string($nrendereco_vjsoc)."'").",".(!$cpendereco_vjsoc ? "NULL" : "'".mysql_real_escape_string($cpendereco_vjsoc)."'").",".(!$cep_vjsoc ? "NULL" : "'".mysql_real_escape_string($cep_vjsoc)."'").",".(!$telefone_vjsoc ? "NULL" : "'".mysql_real_escape_string($telefone_vjsoc)."'").",".(!$cpf_vjsoc ? "NULL" : "'".mysql_real_escape_string($cpf_vjsoc)."'").",".(!$sexo_vjsoc ? "NULL" : "'".mysql_real_escape_string($sexo_vjsoc)."'").",".(!$cod_pais ? "NULL" : "'".mysql_real_escape_string($cod_pais)."'").",".(!$cod_logr ? "NULL" : "'".mysql_real_escape_string($cod_logr)."'").",".(!$cod_bairro ? "NULL" : "'".mysql_real_escape_string($cod_bairro)."'").",".(!$cod_uf ? "NULL" : "'".mysql_real_escape_string($cod_uf)."'").",".(!$cod_municipio ? "NULL" : "'".mysql_real_escape_string($cod_municipio)."'").",".(!$cod_vend ? "NULL" : "'".mysql_real_escape_string($cod_vend)."'")."
			)
		";
		$this->query();
		return $this->qrdata;
