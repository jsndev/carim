...
if($db->qrcount>0){
  ...
  $ecivil      = $forms->getECivil($ecivil);
	...
	$mensagem = new mensagens();
	if($f_situacao > 2 && $f_situacao < 5){
		$mensagem->setMensagem('Esta proposta está em fase de '.$aSIT_PPST[$f_situacao].', disponível apenas para consulta.', MSG_ALERTA);
	}
	?>
	  ...
	  ?>
		...
		<div class="quadroInterno">
	    	...
	    	<div class="warning" id="divResultadoProposta" style="border:1px solid #DDDDDD; background-color: #F5F5F5; padding: 20px; float:left; width:200px;">
		  		Prestação Inicial:</td><td align="left"><b>R$ <?=$f_prestacao;?></b><br />
	  			<? if($f_pricesac_ppst==2){ ?>
	  			Redução Mensal de Juros:</td><td align="left"><b>R$ <?=$f_redMJ;?></b>
	  			<? } ?>
	    	</div>
			</div>
			<div><img src="images/layout/subquadro_b.gif" alt=" " /></div>
