...
Calendar.cellClick = function(el) {
	...
	if (typeof el.navtype == 'undefined') {
		...
		switch (el.navtype) {
			...
			break;
		    case 100:
				cal.callHandlerClean(); // Nova função para este botão... Limpa o campo, deixando ele em branco. // by Ronny Nascimento
				Calendar.removeClass(el, "hilite");
				cal.callCloseHandler();
				// cal.setMondayFirst(!cal.mondayFirst); // esta era a antiga função deste botão... trocar a forma como era mostrada o primeiro dia da semana.. (ou segunda.. ou domingo). ///  by Ronny Nascimento
			return;
		}
				...
				if ((!i && j < wday) || iday > no_days) {
					...
					if (this.disabledHandler && this.disabledHandler(date)) {
						...
						cell.ttip =
							...
							", " + iday +
							" de " + Calendar._MN[month].substr(0, 3) + " de " + year;
						Calendar._add_evs(cell);
						if (iday == mday) {
...
Calendar.prototype.callHandlerClean = function () { // função que limpa o campo de data. 
	if (this.yourHandler) {							// by Ronny Nascimento
		this.yourHandler(this, '');
	}
};
