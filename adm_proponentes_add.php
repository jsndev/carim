...
$pageTitle = "Adicionar Proponente";
...
if ($crypt->decrypt($_POST["ac"]) == "adicionar") {
	...
	$dadosUsuario["level_usua"] = TPUSER_PROPONENTE;
	$dadosUsuario["id_lstn"] = $_POST["matricula"];
	...
	if ($oUsuario->getErrNo() == 0) {
		...
		$mensagem->setMensagem("Proponente cadastrado com sucesso. Um e-mail foi enviado para o proponente contendo sua senha de acesso.", MSG_SUCESSO);
		unset($_POST);
	} elseif ($oUsuario->getErrNo() == DB_ERR_UNIQUE) {
		$mensagem->setMensagem("Já existe ou existiu um proponente cadastrado com esse endereço de e-mail.", MSG_ERRO);
	} else {
		$mensagem->setMensagem("Houve um erro ao cadastrar o proponente. Favor tentar novamente.", MSG_ERRO);
	}
}
?>
...
<div class="quadroInterno">
	...
	<div class="quadroInternoMeio">
		...
		<table cellpadding="0" cellspacing="2" class="tbForm">
			<tr>
				<td>Matrícula: </td>
				<td><input type="text" name="matricula" id="matricula" value="<? echo $_POST["matricula"]; ?>" maxlength="20" /></td>
			</tr>
			<tr>
				<td>Nome: </td>
			...
			<tr>
				...
				<td>
					...
					<p class="warning" style="width: 300px;"><b>Importante:</b> a senha de acesso do proponente será automaticamente gerada e enviada ao endereço de e-mail informado. Certifique-se de que o endereço de e-mail esteja correto.</p>
				</td>
			</tr>
			<tr>
				<td>Ativo: </td>
			...
			<tr>
				<td>&nbsp;</td>
				<td class="alr"><img src="images/buttons/bt_voltar.gif" onclick="goPage('adm_proponentes.php')" alt="Cancelar e voltar" class="cursorMao" />&nbsp;<img src="images/buttons/bt_salvar.gif" onclick="validaProponente(document.getElementById('frm1'));" alt="Inserir Proponente" class="cursorMao" /></td>
			</tr>
		</table>
	</div>
