...
if($acaoProposta=='confirmarAssinatura'){
	// Confirmação de Assinatura do Contrato // -----------------------------------
	$prop_assinatura   = $_POST['prop_assinatura'];
	$f_prop_assinatura = $utils->formataData($prop_assinatura);
	
	$db->query="UPDATE proposta
							SET situacao_ppst=9, dtasscontrato_ppst='".mysql_real_escape_string($f_prop_assinatura)."' 
							WHERE cod_ppst = '".mysql_real_escape_string($_GET['cod_proposta'])."' ";
	$db->query();
	$hist_obs = 'Confirmação de Assinatura do Contrato ('.$prop_assinatura.')';
	$cLOGIN->insert_log(4,7,'Assinatura do Contrato - CODPPST:'.$_GET['cod_proposta']);
	$cLOGIN->insert_history($_GET['cod_proposta'],1,htmlentities($hist_obs));
	// ----------------------------------------------------------------------------
}
...
if($acaoProposta=='salvarImovCartorio'){
	...
	$imov_dt_regist = $utils->formataData($_POST['imov_dt_regist']);
	
	$db->query="UPDATE proposta SET 
								situacao_ppst=10,
								dtokregistro_ppst='".mysql_real_escape_string($imov_dt_regist)."'
							WHERE cod_ppst = '".mysql_real_escape_string($_GET['cod_proposta'])."' ";
	$db->query();
	...
	$db->query();
	//print $db->query.'<hr>';
	$cLOGIN->insert_log(4,7,'Alteração da Proposta - CODPPST:'.$_GET['cod_proposta']);
	$cLOGIN->insert_history($_GET['cod_proposta'],1,'Alteração da Proposta');
}

if($acaoProposta=='confirmarEnvioRemessa'){
	$prop_remessa = $_POST['prop_remessa'];
	$prop_remessa = $utils->formataData($_POST['prop_remessa']);
	$db->query="UPDATE proposta SET 
								situacao_ppst=11,
								dtremessacontrato_ppst='".mysql_real_escape_string($prop_remessa)."'
							WHERE cod_ppst = '".mysql_real_escape_string($_GET['cod_proposta'])."' ";
	$db->query();
	$cLOGIN->insert_log(4,7,'Envio do Contrato e Matricula original à Previ - CODPPST:'.$_GET['cod_proposta']);
	$cLOGIN->insert_history($_GET['cod_proposta'],1,'Envio do Contrato e Matricula original à Previ');
	header('Location:asdmt_lista_prop.php'); exit();
}
...
if($db->qrcount>0){
  ...
  */
  ...
  $f_dt_pgto_boleto = $utils->formataDataBRA($dt_pgto_boleto);
	...
	<script language="JavaScript" src="./js/asdmt.js"></script>
		...
		<div class="quadroInterno">
			...
			<div class="quadroInternoMeio">
	    		...
	    		<table cellpadding=0 cellspacing=5 border=0>
		  			...
		  			<tr>
		  			  <td align="right" valign="top">Prestação:</td><td align="left"><b><?=$f_przfinsol_ppst;?> meses</b></td>
		  			</tr>
		  			<? } ?>
	...
	<div class="quadroInterno">
		...
		<div class="quadroInternoMeio">
	        ...
	        <? if($f_situacao==8){ ?>
	          ...
	          <td align="left"  valign="top">
	            <input type="text" style="width:80px;" name="prop_assinatura" id="prop_assinatura" value="<?=$prop_assinatura;?>" onKeyDown="return teclasInt(this,event);" onKeyUp="return mascaraData(this,event);" maxlength="10">
	          </td>
	          <td align="left"  valign="top">
	            <img src="images/buttons/bt_confirmar.gif" alt="Confirmar Assinatura" class="im" onClick="confirmarAssinatura('<?=$crypt->encrypt('confirmarAssinatura');?>')" />
	          </td>
	        </tr>
	        <? }else{ ?>
	        <tr>
	...
	<div class="quadroInterno">
		...
		<div class="quadroInternoMeio">
	        ...
	        <tr>
	          ...
	          <? if($f_situacao==10){ ?>
	          <td align="left"  valign="top">
	            <input type="text" style="width:80px;" name="prop_remessa" id="prop_remessa" value="<?=$prop_remessa;?>" onKeyDown="return teclasInt(this,event);" onKeyUp="return mascaraData(this,event);" maxlength="10">
	          </td>
	          <td align="left"  valign="top">
	            <img src="images/buttons/bt_confirmar.gif" alt="Confirmar Envio" class="im" onClick="confirmarEnvioRemessa('<?=$crypt->encrypt('confirmarEnvioRemessa');?>')" />
	          </td>
            <? }else{ ?>
	          <td align="left"  valign="top">
	            <b><?=$data_remessa;?></b>
