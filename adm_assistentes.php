...
$pageTitle = "Assist. Administrativos";
...
if ($_GET["dmy"] != "" && $_GET["cod_usua"] != "" && $_GET["ac"] == "del") {
	...
	if ($oUsuario->getErrNo() == 0) {
		$oMensagem->setMensagem("O assistente foi removido com sucesso", MSG_SUCESSO);
	} else {
		$oMensagem->setMensagem("O assistente não pôde ser removido. Tente novamente.", MSG_ERRO);
	}
	...
	if ($oUsuario->getErrNo() == 0) {
		...
		$email->send();
		$oMensagem->setMensagem("A senha do assistente \"".$dadosUsuario[0]["nome_usua"]."\" foi alterada com sucesso. Um e-mail foi enviado ao seu endereço eletrônico contendo sua nova senha de acesso.", MSG_SUCESSO);
	} else {
		$oMensagem->setMensagem("A senha do assistente \"".$dadosUsuario[0]["nome_usua"]."\" não pôde ser alterada. Tente novamente.", MSG_ERRO);
	}
}

$aListaUsuario = $oUsuario->getListaUsuarios(TPUSER_ADMINISTRATIVO);
?>
<script language="javascript" type="text/javascript" src="js/cadastroAssistente.js"></script>
...
?>
					<tr class="tL<? echo $nIdxDadoUsuario%2 ? "1" : "2"; ?>">
						...
						<td class="alc"><a href="javascript:delAssistente('<? echo eregi_replace("\'", "", $aDadoUsuario["nome_usua"]); ?>', '<? echo $crypt->encrypt("ac=del&dmy=".md5(mt_rand(0,(($nIdxDadoUsuario+1)*(int)mktime())))."&cod_usua=".$aDadoUsuario["cod_usua"]); ?>');" title="Excluir"><img src="images/buttons/bt_excluir.gif" alt="Excluir" /></a>&nbsp;<a href="adm_assistentes_alt.php?k=<? echo $crypt->encrypt("cod_usua=".$aDadoUsuario["cod_usua"]); ?>" title="Alterar"><img src="images/buttons/bt_alterar.gif" alt="Alterar" /></a>&nbsp;<a href="javascript:altPwdAssistente('<? echo eregi_replace("\'", "", $aDadoUsuario["nome_usua"]); ?>', '<? echo $crypt->encrypt("ac=uppwd&dmy=".md5(mt_rand(0,(($nIdxDadoUsuario+1)*(int)mktime())))."&cod_usua=".$aDadoUsuario["cod_usua"]); ?>');" title="Alterar Senha"><img src="images/buttons/bt_alterarsenha.gif" alt="Alterar Senha" /></a></td>
					</tr>
<?
	}
}
?>
				...
				<tfoot>
					<tr>
						<td colspan="5"><a href="adm_assistentes_add.php"><img src="images/buttons/bt_adicionar.gif" alt=" " /></a></td>
					</tr>
				</tfoot>
			</table>
		</div>
