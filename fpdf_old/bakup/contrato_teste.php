...
if($tipo_ppst=='S'){ // Qualificação do Participante de Proposta Simples
	...
	$entrada_finan	= $registro['VLENTRAPROVADO'];
		...
		//________________________________  Valor Prestação _______________________
	$query = "SELECT SUM(PARCAPROVADA) AS total FROM proponente,listadenomes,usuario WHERE proponente.cod_ppst='".$cod_ppst."' AND listadenomes.id_lstn=usuario.id_lstn AND usuario.cod_usua=proponente.COD_PROPONENTE ORDER BY total DESC";
	$result =mysql_query($query);
	$registro = mysql_fetch_array($result, MYSQL_ASSOC);
	$parcela_finan = $registro['total'];	
...
##########################################################################################################		
		...
		$quadro_sete="<b>
					...
					<table border='1' width='728'>
						...
						<tr>
							<td>7. Data de Vencimento do primeiro pagamento mensal: 20/".proxMes($asscontrato)."/2008</td>
						</tr>
						<tr>
		...
		$quadro_nove="<b>
					...
					<table border='1' width='728'>
						...
						<tr>
							<td>2. Data do pagamento: 20/".proxMes($asscontrato)."/2008</td>
						</tr>
					</table><br>";
		$quadro_dez="<b>
		...
		$fixa=" <hr>
				...
				<p align='right'><br><br>São Paulo - ".data_extenso($asscontrato).".</p>
				<br>
				...
				<tr>
					<td width='200'></td>
					<td width='364' align='center'><b>P.P. CAIXA DE PREVIDÊNCIA DOS FUNCIONÁRIOS DO BANCO DO BRASIL - HELENA ALEGRETTI GALLIERA ABOLAFIO</b></td>
				</tr>
			</table>
				<br>
