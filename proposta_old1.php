...
if ($_POST) {
	...
	if (isset($_POST['acao']) && $_POST['acao'] == 'buscar_subtipos') {

		$documentoDAO = new documentoDAO();
		$subtipos = $documentoDAO->buscarSubtipos($_POST['tipo_id']);
		...
		if ($subtipos) {
			...
			foreach ( $subtipos as $subtipo ) {
				...
				$html .= "<option remover='true' value='{$subtipo['id']}'>" . utf8_encode($subtipo['descricao']) . "</option>";
...
{
	//para atualizar o c�digo da proposta na agenda
	$oAgenda->putCodProposta($aProposta["cod_ppst"]);
	//para transpotar o hist�rico
...
return '<?=date('d/m/Y',time());?>'
	...
	<div style="border:3px double #FFF; background-color:#000; color:#FFF; margin-bottom:5px; padding:3px;">
		...
		PREVI:         <b style="color:#FF0;"><?php echo ((FLG_PREVI)?'SIM':'N�O');?></b>
	</div>
	<?php /* -------------------------------------------------------------------------- */ ?>
	
	<?php
    /**
     * Solicitada avaliacao do imovel - ID 3 ( 7 dias uteis - atraso avaliadora )
     * Recebida documenta��o de 2� fase com prazo para an�lise de 5 dias �teis - ID 5 (  5 dias uteis ( atraso ATHOS ) )
     * Recebida documenta��o faltante. Prazo para an�lise de 2 dias �teis - ID 9 (2 uteis ( atraso ATHOS)
     * Encaminhado e-mail de apto. Segue para operacionaliza��o - ID 7 - 2 dias
     * Proposta operacionalizada, encaminhada para qualifica��o - ID 10 - 3 dias atraso athos
     */
    $data_atual = new DateTime('now');
    $atualizacao_historico = new DateTime($aProposta["historico_motivo_data_atualizacao"]);
    $historico_motivo_id = $aProposta["historico_motivo_id"];

    $atraso = [];
    switch ($historico_motivo_id) {

        case 3:
            $atraso = [
                'motivo' => utf8_decode('Solicitada avalia��o do im�vel'),
                'tempo_atraso' => ($data_atual->diff($atualizacao_historico)->days - 7),
                'atrasado' => $data_atual->diff($atualizacao_historico)->days > 7,
                'cor' => 'yellow',
                'responsavel' => 'Avaliadora'
            ];
        ...
        case 5:
            $atraso = [
                'motivo' => utf8_decode('Recebida documenta��o de 2� fase'),
                'tempo_atraso' => $data_atual->diff($atualizacao_historico)->days - 5,
                'atrasado' => $data_atual->diff($atualizacao_historico)->days > 5,
                'cor' => 'red',
                'responsavel' => 'Athos Gest�o'
            ];
        ...
        case 9:
            $atraso = [
                'motivo' => utf8_decode('Recebida documenta��o faltante'),
                'tempo_atraso' => $data_atual->diff($atualizacao_historico)->days - 2,
                'atrasado' => $data_atual->diff($atualizacao_historico)->days > 2,
                'cor' => 'red',
                'responsavel' => 'Athos Gest�o'
            ];
        ...
        case 7:
            $atraso = [
                'motivo' => utf8_decode('Encaminhado e-mail de apto. Segue para operacionaliza��o'),
                'tempo_atraso' => $data_atual->diff($atualizacao_historico)->days - 2,
                'atrasado' => $data_atual->diff($atualizacao_historico)->days > 2,
                'cor' => 'yellow',
                'responsavel' => 'Athos Gest�o'
        ...
        case 10:
            $atraso = [
                'motivo' => utf8_decode('Proposta operacionalizada, encaminhada para qualifica��o'),
                'tempo_atraso' => $data_atual->diff($atualizacao_historico)->days - 3,
                'atrasado' => $data_atual->diff($atualizacao_historico)->days > 3,
                'cor' => 'red',
                'responsavel' => 'Athos Gest�o'
            ];
    ...
    <?php if (!empty($atraso) && $atraso['tempo_atraso'] > 0) : ?>
        <div style="font-size: 21px; background-color: <?php echo $atraso['cor'] ?>;  padding: 10px; font-variant: all-small-caps;">
            <?php echo $atraso['motivo'] ?> com <?php echo $atraso['tempo_atraso'] ?> dias em atraso. <br>
            Respons�vel: <?php echo $atraso['responsavel'] ?>
        </div>
    <?php endif; ?>
	...
	<?php
		...
		if( in_array($aProposta["situacao_ppst"],$aPROPOSTALISTA[$cLOGIN->iLEVEL_USUA]) ){
			?>
			<form enctype="multipart/form-data" name="proposta" id="proposta" method="post" action="proposta.php<?php echo (($cLOGIN->iLEVEL_USUA>1)?'?cod_proposta='.$aProposta["cod_ppst"]:'');?>">
...
function SalvarResponsavel(){
		...
		alert('Por favor, informe o respons�vel pela pasta!');
		return false;
		}
		else if((!vCheck('locresp',   ' de onde � o respons�vel'))){
		return false;
		}
		else{
...
function verificaexig()
{
	if(document.getElementById('descexig'))
	{
		...
		{
			alert('Preencha a Descri��o da Exigencia')
			return false;
		}else
		{
			return true;
...
?>	
	<tr style="<?php echo $mostra;?>">
	  <td align="left" valign="top">In�cio de Exig�ncia:&nbsp;&nbsp;&nbsp;<b><?php echo DataBRA($registro['DTINIEXIGENCIA_PPST']);?></b>
	  							   &nbsp;&nbsp;&nbsp; Fim de Exig�ncia:&nbsp;&nbsp;&nbsp;<input type="text" name="<?php echo $fim_exig;?>" id="<?php echo $fim_exig;?>"  value="" onKeyDown="return teclasInt(this,event);" onKeyUp="return mascaraData(this,event);">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	  </td>
	</tr>
	<tr style="<?php echo $mostra;?>">
...
?>	
	<tr style="<?php echo $mostra;?>">
	  <td align="left" valign="top">In�cio de Exig�ncia:&nbsp;&nbsp;&nbsp;<b><?php echo DataBRA($registro['DTINIEXIGENCIA_PPST']);?></b>
	  							   &nbsp;&nbsp;&nbsp; Fim de Exig�ncia:&nbsp;&nbsp;&nbsp;<b><?php echo DataBRA($registro['DTFIMEXIGENCIA_PPST']);?></b>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	  </td>
	</tr>
<?php }
...
?>
	<tr >
	  <td align="left"><input <?php if ($prop_addexig==1){echo "checked";}?> type="radio" class="rd" name="<?php echo $flg_exigencia;?>" id="<?php echo $flg_exigencia;?>" value="1" onclick="atualexig('prop_exigencia');" >&nbsp;&nbsp;&nbsp;<b>Adicionar Exig�ncia</b></td>
<?php 
...
<tr>
	  <td align="right" valign="top"><img src="images/buttons/bt_salvar.gif"   id="<?php echo $bt_exigencia;?>"   alt="Salvar Exig�ncia" class="im" onClick="return salvarExig('<?php echo $hd_botao;?>');" /> </td>
	</tr> <?php 
	}
}?>	
	</table>
...
<div>
	...
	<script src="tinymce2/js/tinymce/jquery.tinymce.min.js"></script>
	<script src="tinymce2/js/tinymce/tinymce.min.js"></script>
	<script>
	$(document).ready(function(){
		tinyMCE.init({
			selector: '#contrato_field',
			theme: 'modern',
				...
				jQuery.ajax({
					...
					success: function(response) {
						...
						if (response == false) {
							...
							alert('Problemas ao atualizar, entre em contato com a TI.');

						} else {

							jQuery('#message').html('Registro salvo com sucesso.')
							jQuery('#message').show();
...
<div class="bloco_include">
	...
	<div class="quadroInterno">
		...
		<div class="quadroInternoMeio">
					 ...
					 ?>
                     <tr>
		         	 <td width="158" align="right" valign="top"><b>Respons�vel pela Pasta:</b></td>
...
<?php
			...
			$oMensagem->setMensagem("Nenhuma proposta associada ao usu�rio",MSG_ERRO);
