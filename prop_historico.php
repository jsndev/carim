...
$aProposta = $oProposta->getProposta(18);
//print_r($aProposta);
utils::printArray($aProposta);
$oProposta->upd(util::campos($_POST,"proposta"));
*/
...
*/
  ...
  $ecivil      = $forms->getECivil($ecivil);
	...
	$i_cod_uf    = $db->qrdata[0]["iuf"];
	$i_cod_mun   = $db->qrdata[0]["imun"];
	$i_uf        = $forms->getUF($i_cod_uf);
  ...
  $f_dt_pgto_boleto = $utils->formataDataBRA($dt_pgto_boleto);
	...
	## checklist ###################################################################
	if($_POST) {
		if($acaoProposta=='salvarPropostaChkLst'){
		  foreach ($_POST['ckl_doc_dt_ped'] as $key=>$value) {
				$c_check=($_POST['ckl_doc_check'][$key]?1:0);
				$c_dt_ped=(!empty($_POST['ckl_doc_dt_ped'][$key])?"'".mysql_real_escape_string(utils::formataData($_POST['ckl_doc_dt_ped'][$key]))."'":"NULL");
				$c_dt_emis=(!empty($_POST['ckl_doc_dt_emis'][$key])?"'".mysql_real_escape_string(utils::formataData($_POST['ckl_doc_dt_emis'][$key]))."'":"NULL");
				...
				$db->query="replace into checklist(
								...
								'".mysql_real_escape_string($cod_ppst)."',
								'".mysql_real_escape_string($i_cod_mun)."',
								'".mysql_real_escape_string($i_cod_uf)."',
								".$c_dt_ped.",
								".$c_dt_emis.",
		...
		if($acaoProposta=='salvarImovCartorio'){
			...
			$imov_dt_regist = $_POST['imov_dt_regist'];
			$f_imov_dt_regist = $utils->formataData($imov_dt_regist);
			
			$db->query="UPDATE proposta SET 
										situacao_ppst=10,
										dtokregistro_ppst='".mysql_real_escape_string($f_imov_dt_regist)."'
									WHERE cod_ppst = '".mysql_real_escape_string($cod_ppst)."' ";
			$db->query();
			$f_situacao = 10;
		
			$db->query="UPDATE imovel SET
										nomecartrgi_imov='".mysql_real_escape_string($imov_cartr_rgi)."',
										nrmatrgi_imov='".mysql_real_escape_string($imov_matrc_rgi)."',
										nrlivrgi_imov='".mysql_real_escape_string($imov_livro_rgi)."',
										nrfolhrgi_imov='".mysql_real_escape_string($imov_folhs_rgi)."',
										nrrgcompvend_imov='".mysql_real_escape_string($imov_rg_cprvnd)."',
										nrrggar_imov='".mysql_real_escape_string($imov_rg_garant)."'
									WHERE cod_ppst = '".mysql_real_escape_string($cod_ppst)."' ";
			$db->query();
			//print $db->query.'<hr>';
			$cLOGIN->insert_log(4,7,'Alteração da Proposta - CODPPST:'.$cod_ppst);
			$cLOGIN->insert_history($cod_ppst,1,'Alteração da Proposta');
		}
	}else{
		$cLOGIN->insert_log(4,7,'Visualização da Proposta - CODPPST:'.$cod_ppst);
	}
	################################################################################
	...
	<form name="proposta" id="proposta" method="post" action="<?=$php_self;?>">
	  ...
	  ?>
 		<br><?=$mensagem_post->getMessageBox();?>
					...
					<div class="quadroInternoMeio">
				    		<table cellpadding=0 cellspacing=5 border=0 width="100%">
				    			...
				    			<tr>
				    			  ...
				    			  <? if($f_situacao <= 5){ ?>
				    			  <td align="right" rowspan="2" valign="bottom"><img src="images/buttons/bt_gerar_boleto.gif" alt="Gerar Boleto" class="im" onClick="gerarBoleto('<?=$cod_ppst;?>');" /></td>
				    			  <? } ?>
				    			</tr>
				    			<tr>
	  ...
	  <br><b>Dados do Imóvel</b>
	    ...
	    ### DADOS DO VENDEDOR ##################################################################### 
		  ...
		  if($db->qrcount>0){
			  ...
			  if($vf_rendim) $vf_rendim  = $utils->formataMoeda($vf_rendim);
		...
		<div class="quadroInterno">
			...
			<div class="quadroInternoMeio">
	    			...
	    			<? }elseif($vend_tipo==2){ ?>
	    				...
	    				<?
							  $db->query="select * from vendjursocio where cod_ppst = '".mysql_real_escape_string($cod_ppst)."' ";
							  $db->query();
							  if($db->qrcount>0){
    ...
    $db->query();
			...
			<div class="quadroInternoMeio">
				...
				<div class="tListDiv">
					<table>
						...
						<tbody>
						<?
						$disab_chk_lst = ($despachante=='' && $f_situacao>=3 && $f_situacao<=5)?'':'disabled';
						if(count($aDADOSDOCUMENTOS)>0){
							for($i=0; $i<count($aDADOSDOCUMENTOS); $i++){
								?>
								<tr class="tL<? echo $i%2 ? "1" : "2"; ?>">
									<td class="alc"><input type="checkbox" class="ck" name="ckl_doc_check[<?=$aDADOSDOCUMENTOS[$i]["cod_docm"];?>]" value="1" id="ckl_doc_check[<?=$aDADOSDOCUMENTOS[$i]["cod_docm"];?>]" <?=($aDADOSDOCUMENTOS[$i]["flgstatus_clst"]?"checked":"")?> <?=$disab_chk_lst;?>></td>
									...
									<? if($despachante=='' && $f_situacao>=3 && $f_situacao<=5){ ?>
										<td class="alc">
									...
									<td><textarea style="width:150px; height:30px;" name="ckl_doc_desc[<?=$aDADOSDOCUMENTOS[$i]["cod_docm"];?>]" id="ckl_doc_desc[<?=$aDADOSDOCUMENTOS[$i]["cod_docm"];?>]" <?=$disab_chk_lst;?>><?=$aDADOSDOCUMENTOS[$i]["obs_clst"];?></textarea></td>
								</tr>
								<?
							}
						} else {
	  ...
	  <?
	    ...
	    if($despachante=='' && $f_situacao>=3 && $f_situacao<=5){ ?>
			    ...
			    <input type="image" name="btSalvar" id="btSalvar"   src="images/buttons/bt_salvar.gif"   value="Salvar"   class="im" onClick="return salvarPropostaChkLst('<?=$crypt->encrypt('salvarPropostaChkLst');?>',<? echo ($situacao ? $situacao : '1'); ?>);" />
			  </div>
		  <? } ?>
		  <? if($f_situacao==9){ ?>
				<div style="text-align:center; margin-top:10px;">
			    <img src="images/buttons/bt_salvar.gif"   alt="Salvar"   class="im cursorMao" onClick="salvarProposta('<?=$crypt->encrypt('salvarImovCartorio');?>');" />
			  </div>
		  <? } ?>
