...
if(empty($_GET["cod_proposta"])){
	header("Location: ajurd_lista_prop.php"); exit();
}
...
if($acaoProposta=='evento'){
	...
	// ----------------------------------------------------------------------------
}elseif($acaoProposta=='confirmarRetornar'){
	$db->query="UPDATE proposta SET situacao_ppst=5 WHERE cod_ppst = '".mysql_real_escape_string($_GET['cod_proposta'])."' ";
	$db->query();
	$f_motivo_retorno = '<b>Retornar para Analise Documental</b><br>'.htmlentities($_POST['motivo_retorno']);
	$cLOGIN->insert_log(4,7,'Retornar para Analise Documental - CODPPST:'.$_GET['cod_proposta']);
	$cLOGIN->insert_history($_GET['cod_proposta'],2,$f_motivo_retorno);
	header("Location: ajurd_lista_prop.php"); exit();
}elseif($acaoProposta=='aprovarContrato'){
	$db->query="UPDATE proposta SET situacao_ppst=7, dtaprovacao_ppst=now() WHERE cod_ppst = '".mysql_real_escape_string($_GET['cod_proposta'])."' ";
	$db->query();
}elseif($acaoProposta=='confirmarAgendamento'){
	$data_agendam = $utils->formataData($_POST['prop_agendam']);
	$db->query="UPDATE proposta SET dtagend_asscontr_ppst='".$data_agendam."', situacao_ppst=8 WHERE cod_ppst = '".mysql_real_escape_string($_GET['cod_proposta'])."' ";
	$db->query();
}elseif($acaoProposta=='parecerFinal'){
	$db->query="UPDATE proposta SET situacao_ppst=12 WHERE cod_ppst = '".mysql_real_escape_string($_GET['cod_proposta'])."' ";
	$db->query();
	$f_parecer = $_POST['parecer'];
	$cLOGIN->insert_log(4,7,'Parecer Final - CODPPST:'.$_GET['cod_proposta']);
	$cLOGIN->insert_history($_GET['cod_proposta'],5,htmlentities($f_parecer));
	header('Location:ajurd_lista_prop.php'); exit();
}
...
$db->query="SELECT
            ...
            WHERE u.cod_usua = p.proponente_ppst
              ...
              AND p.cod_ppst = i.cod_ppst
             	AND p.cod_ppst = '".mysql_real_escape_string($_GET['cod_proposta'])."'
...
if($db->qrcount>0){
  ...
  $flgAprovPrevi  = $db->qrdata[0]['flgaprovacaoprevi_ppst'];
  ...
  */
  ...
  $ecivil      = $forms->getECivil($ecivil);
	...
	if(($f_situacao < 6 || $f_situacao > 7) && $f_situacao != 11){
		//print $f_situacao; exit();
		header("Location: ajurd_lista_prop.php"); exit();
	}
  ...
  $db->query();
    ...
    $imov_cep      = $utils->formataCEP($imov_cep);
		...
		$i_cod_mun   = $f_i_cidade;
		$f_i_uf      = $forms->getUF($f_i_uf);
		$f_i_uf      = trim($f_i_uf[0]['nome_uf']);
  ...
  $f_dt_pgto_boleto = $utils->formataDataBRA($dt_pgto_boleto);
	...
	if($flgAprovPrevi!='S'){
		$mensagem->setMensagem('Aguardando a aprovação da proposta pela Previ!', MSG_ALERTA);
	}
	...
	<script language="javascript">
		...
		var flgAprovPrevi = '<?=$flgAprovPrevi;?>';
	</script>

	<script language="JavaScript" src="./js/diversos.js"></script>
	<script language="JavaScript" src="./js/ajurd.js"></script>
...
*/ ?>
  ...
  <br><b>Check List</b>
	...
	<div style="width:500px; text-align:right; margin-top:10px;">
		...
		<? if($f_situacao==6){ ?>
			<img src="images/buttons/bt_retorn_anal_doc.gif" alt="Retornar para Análise Documental" class="im" onClick="retornarAnalDoc();" />
			<img src="images/buttons/bt_aprovar_contrato.gif" alt="Aprovar Contrato" class="im" onClick="aprovarContrato('<?=$crypt->encrypt('aprovarContrato');?>');" />
		<? }elseif($f_situacao==7){ ?>
			<img src="images/buttons/bt_imprimir_contrato.gif" alt="Imprimir Contrato" class="im" onClick="imprimirContrato();" />
			<img src="images/buttons/bt_agendar_assinatura.gif" alt="Agendar Assinatura" class="im" onClick="agendarAssinatura();" />
		<? } ?>
	</div>
	  ...
	  <br><b>Agendar Assinatura</b>
		<div class="quadroInterno">
	      ...
	      <table cellpadding=0 cellspacing=5 border=0>
	      	...
	      	<tr>
	          <td align="right" valign="top">Data da Assinatura do Contrato:<?=$obrig;?></td>
	          <td align="left"  valign="top">
	            <input type="text" style="width:80px;" name="prop_agendam" id="prop_agendam" value="<?=$prop_agendam;?>" onKeyDown="return teclasInt(this,event);" onKeyUp="return mascaraData(this,event);" maxlength="10">
	          </td>
	          <td align="left"  valign="top">
	            <img src="images/buttons/bt_confirmar.gif" alt="Confirmar Agendamento" class="im" onClick="confirmarAgendamento('<?=$crypt->encrypt('confirmarAgendamento');?>')" />
	          </td>
	        </tr>
  ...
  <div id="motivo" style="display:none;">
	  <a name="motivo"></a>
	  <br><b>Retornar para Análise Documental</b>
		<div class="quadroInterno">
			<div><img src="images/layout/subquadro_t.gif" alt=" " /></div>
			<div class="quadroInternoMeio">
	      <table cellpadding=0 cellspacing=5 border=0>
	      	...
	      	<tr>
	          <td align="right" valign="top">Motivo:<?=$obrig;?></td>
	          <td align="left"  valign="top"><textarea style="width:500px; height:60px;" name="motivo_retorno" id="motivo_retorno"></textarea></td>
	        </tr>
	        <tr>
	          <td align="right" valign="top">&nbsp;</td>
	          <td align="left"  valign="top">
	            <img src="images/buttons/bt_confirmar.gif" alt="Confirmar" class="im" onClick="confirmarRetornar('<?=$crypt->encrypt('confirmarRetornar');?>')" />
	          </td>
	        </tr>
  ...
  <?
    ...
    if($f_situacao<11){
	  ...
	  <br><b>Nova Ação</b>
		<div class="quadroInterno">
	        ...
	        <tr>
	          <td align="right" valign="top">&nbsp;</td>
	          <td align="left"  valign="top"><img name="btAddEvnt" id="btAddEvnt"   src="images/buttons/bt_adicionar.gif" value="Adicionar Evento" class="im" onClick="return addEvento('<?=$crypt->encrypt('evento');?>');" /></td>
	        </tr>
	      </table>
			</div>
			<div><img src="images/layout/subquadro_b.gif" alt=" " /></div>
    ...
    if($f_situacao==11){
	  ?>
	  <a name="parecerfinal"></a>
	  <br><b>Parecer Final</b>
		<div class="quadroInterno">
			<div><img src="images/layout/subquadro_t.gif" alt=" " /></div>
			<div class="quadroInternoMeio">
				<table cellpadding=0 cellspacing=5 border=0>
	        <tr>
	          <td align="right" valign="top">Parecer:</td>
	          <td align="left"  valign="top"><textarea style="width:500px; height:60px;" name="parecer" id="parecer"></textarea></td>
	        </tr>
	        <tr>
	          <td align="right" valign="top">&nbsp;</td>
	          <td align="left"  valign="top"><img name="btAddEvnt" id="btAddEvnt"   src="images/buttons/bt_concluir.gif" value="Concluir" class="im" onClick="return parecerFinal('<?=$crypt->encrypt('parecerFinal');?>');" /></td>
	        </tr>
	      </table>
			</div>
			<div><img src="images/layout/subquadro_b.gif" alt=" " /></div>
