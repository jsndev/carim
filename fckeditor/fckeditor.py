...
class FCKeditor(object):
	...
	def CreateHtml(self):
	    ...
	    if (self.IsCompatible()):
	        File = "fckeditor.html"
	        Link = f"{self.BasePath}editor/{File}?InstanceName={self.InstanceName}"
	        if (self.ToolbarSet is not None):
	            Link += f"&amp;Toolbar={self.ToolbarSet}"
	        ...
	        WidthCSS = f"{self.Width}px" if (self.Width.find("%%") < 0) else self.Width
	        HeightCSS = f"{self.Height}px" if (self.Height.find("%%") < 0) else self.Height
