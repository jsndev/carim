						...
						<table style="width:687px;">
							...
							<?
								for($i=0; $i<$db->qrcount; $i++){
									...
									if($cancelam!=''){
										$estilo = 'class="indCancel"';
										$lnk_acao = 'consulta';
										$status   = $_SESSION["indic_cancel"][$cancelam];
									}
									?>
										<tr class="tL<? echo $i%2 ? "1" : "2"; ?>">
											...
											<td <?=$estilo;?> ><? echo $status; ?></td>
											<td><a href='proposta.php?cod_proposta=<?=$db->qrdata[$i]['cod_ppst'];?>'><? echo $lnk_acao; ?></a></td>
										</tr>
									<?
								}
							?>
