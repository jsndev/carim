...
<input type="radio" class="rd" name="flg_intv" id="flg_intv" value="S" <?=($flg_intv=='S')?'checked':'';?> onclick="trocouTemInvQuitante(this);" /> <b>Sim</b> &nbsp;&nbsp;<input type="radio" class="rd" name="flg_intv" id="flg_intv" value="N" <?=($flg_intv=='N')?'checked':'';?> onclick="trocouTemInvQuitante(this);" /><b>Não</b>
							...
							<table cellpadding=0 cellspacing=5 border=0 width="100%">
												...
												jQuery.ajax({
													...
													success: function(response){
														...
														if (!response.erro) {
															...
															jQuery('#endereco_intq').val(response.logradouro);
															jQuery('#cpendereco_intq').val(response.complemento);

															//encontra o bairro
															jQuery('#cod_bairro_intq').find('option').each(function(i, value){
															...
															//encontra o estado
															jQuery('#cod_uf_intq').find('option').each(function(i, value){

																if ( jQuery.trim(jQuery(value).text().toLowerCase()) == response.uf.toLowerCase() ) {
																	jQuery(value).attr('selected', 'selected');

																	jQuery('#cod_uf_intq').change();

																	setTimeout(function(){

																		//encontra a cidade
																		jQuery('#cod_municipio_intq').find('option').each(function(i, value){
								...
								<tr>
									...
									<td align="left"  valign="top">
										<input type="text" class="cep" style="width:150px;" name="cep_intq" id="cep_intq" value="<?php echo $utils->formataCep($cep_intq);?>" onKeyDown="return teclasInt(this,event);" onKeyUp="return mascaraCEP(this,event);" maxlength="9">
									</td>
								</tr>
								<tr>
...
<a name="pagamento"></a>
<div class="bloco_include" id="bloco_pagamento">
	<div class="bloco_titulo">Qualifica??o de Interveniente Quitante</div>
	<div class="quadroInterno">
		<div><img src="images/layout/subquadro_t.gif" alt="" /></div>
		<div class="quadroInternoMeio">
		    		<table cellpadding=0 cellspacing=5 border=0 width="100%">
		    			<colgroup><col width="150" /><col width="*" /></colgroup>
							  <tr>
								<td align="right" valign="top">Interveniente Quitante:	</td><td><?php
										...
										if($flg_intv=='S') echo "<b>Sim</b>"; else echo "<b>N?o</b>";
