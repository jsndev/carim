					...
					if( is_array($vckPpnt) ){
						...
						if($just_display){
							...
							<tr class="tL<? echo $i%2 ? "1" : "2"; ?>">
								<td class="alc"><img src="images/layout/ck_<?=($vckPpnt["dados"]["flgdespachante_clpn"]=='S'?"true":"false")?>.gif" /></td>
								<td class="alc"><img src="images/layout/ck_<?=($vckPpnt["dados"]["flgatendente_clpn"]=='S'?"true":"false")?>.gif" /></td>
								<td><?=$vckPpnt["documento_mndc"];?></td>
								<td class="alc"><?=$utils->formataDataBRA($vckPpnt["dados"]["dtsolicitacao_clpn"]);?></td>
								<td class="alc"><?=$utils->formataDataBRA($vckPpnt["dados"]["dtemissao_clpn"]);?></td>
