<?php
$displayDetsImovel = 'display:none;';

if(isset($_POST['imov_addexig'])){
	$displayDetsImovel = '';
}
...
ver=1;
function showDetsImovel(){
			if(ver){
			document.getElementById("div_add_imov").style.display='block'
			ver=0;
			}
			else{
			document.getElementById("div_add_imov").style.display='none'
			ver=1;
			}
}
...
<div class="bloco_include">
	...
	<div class="quadroInterno">
		...
		<div class="quadroInternoMeio">
                <div class="tListDiv" style="width:auto;">
                      ...
                      <table style="width:704px;">
                        ...
                        <tr class="tL1"><td>
                        <?php
                        if($aProposta["imovel"]["endereco_imov"]!=''){
						?>
						<?=$aProposta["imovel"]["logradouro"][0]["desc_logr"];?>:
                        <?=$aProposta["imovel"]["endereco_imov"];?> n?
						<?=$aProposta["imovel"]["nrendereco_imov"];?>
						<?php
						if($aProposta["imovel"]["cpendereco_imov"]!=''){
						?>
						- <?=$aProposta["imovel"]["cpendereco_imov"];?>
						<?php
						}
						?>,
                        <?=$aProposta["imovel"]["bairro"][0]["nome_bairro"];?>,
                        <?=$aProposta["imovel"]["municipio"][0]["nome_municipio"];?>-<?=$aProposta["imovel"]["uf"][0]["nome_uf"];?>      <?php
                        }
						else{
						echo "Não informado";
						}
                ...
                if(
					...
					&& $aProposta["imovel"]["dtaprovacao_imov"]==''
				){
				?>
                <img src="images/buttons/bt_alterarmini.gif"  alt="Alterar"  class="im" onclick="showDetsImovel();" />
                    <?php
					  }
					  else{
					  ?>  
   				<img src="images/buttons/bt_visualizarmini.gif" alt="Detalhes" class="im" onclick="showDetsImovel();" />
							...
							jQuery.ajax({
								...
								success: function(response){
									...
									if (!response.erro) {
										...
										jQuery('#endereco_imov').val(response.logradouro);
										jQuery('#cpendereco_imov').val(response.complemento);
										jQuery('#bairro_ppnt').val(response.bairro);

										//encontra o bairro
										jQuery('#cod_bairro_imov').find('option').each(function(i, value){
										...
										//encontra o estado
										jQuery('#cod_uf_imov').find('option').each(function(i, value){

											if ( jQuery.trim(jQuery(value).text().toLowerCase()) == response.uf.toLowerCase() ) {
												jQuery(value).attr('selected', 'selected');

												jQuery('#cod_uf_imov').change();

												setTimeout(function(){

													//encontra a cidade
													jQuery('#cod_municipio_imov').find('option').each(function(i, value){
		...
		<div id="div_add_imov" style="<?=$displayDetsImovel;?>">
			...
			?>		<div id="div_add_imov" >
				...
				<div style="float:left; width:500px;">
					<table cellpadding=0 cellspacing=5 border=0 id="tbEditImov">
						<tr>
							...
							<td align="left"  valign="top"><input type="text" class="cep" style="width:100px;" name="cep_imov" id="cep_imov" value="<?=$utils->formataCep($aProposta["imovel"]["cep_imov"]);?>" onKeyDown="return teclasInt(this,event);" onKeyUp="return mascaraCEP(this,event);" maxlength="9" /></td>
						</tr>
		  			<tr>
	    		...
	    		<table cellpadding=0 cellspacing=5 border=0>
	    			...
	    			<tr>
	    			  <td align="right" valign="top">Estado Conservação Im?vel:<? $utils->obrig('estconserv_imov'); ?></td>
	    			  <td align="left">
		    			...
		    			<tr>
		    			  ...
		    			  <td align="left"  valign="bottom">
		    			  	<? $utils->obrig('tmbdspcndop_imov'); ?>
								  <?
								 $aProposta["imovel"]["tmbdspcndop_imov"]=($aProposta["imovel"]["tmbdspcndop_imov"]=='')?'N':$aProposta["imovel"]["tmbdspcndop_imov"];
		    			...
		    			<tr>
		    			  ...
		    			  <td align="left"  valign="bottom">
		    			  	<? $utils->obrig('incomb_imov'); ?>
								  <?
							 $aProposta["imovel"]["incomb_imov"]=($aProposta["imovel"]["incomb_imov"]=='')?'S':$aProposta["imovel"]["incomb_imov"];
								  	foreach ($listas->getListaSN() as $k=>$v){
		    			...
		    			<tr>
		    			  ...
		    			  <td align="left"  valign="bottom">
		    			  	<? $utils->obrig('ruralfav_imov'); ?>
								  <?
								$aProposta["imovel"]["ruralfav_imov"]=($aProposta["imovel"]["ruralfav_imov"]=='')?'N':$aProposta["imovel"]["ruralfav_imov"];
								  	foreach ($listas->getListaSN() as $k=>$v){
		    			...
		    			<tr>
		    			  ...
		    			  <td align="left"  valign="bottom">
		    			  	<? $utils->obrig('emconstr_imov'); ?>
								  <?
								  $aProposta["imovel"]["emconstr_imov"]=($aProposta["imovel"]["emconstr_imov"]=='')?'N':$aProposta["imovel"]["emconstr_imov"];
								  	foreach ($listas->getListaSN() as $k=>$v){
							...
							?>
								<div class="divVagaGaragem">
									...
									<table cellpadding=0 cellspacing=5 border=0>
										...
										<tr>
											<td align='right' valign='top'>?rea total (m?):<? $utils->obrig('vagas_garagem'); ?></td>
										...
										<tr id='tr_num_matr_vaga_imov_<?=$vg;?>' style='<?=$display_dados_vaga;?>'>
											<td align='right' valign='top'>Número de Matr?cula:<? $utils->obrig('vagas_garagem'); ?></td>
				...
				<? if($cLOGIN->iLEVEL_USUA==2){ ?>
				  <div class="grupoDados" style="width:450px;">
						...
						<tr>
			  			  ...
			  			  <td align="left"><input type="text" style="width:80px;" name="vlavaliacao_imov" id="vlavaliacao_imov" value="<?=$utils->formataMoeda($aProposta["imovel"]["vlavaliacao_imov"]);?>" onKeyDown="return teclasInt(this,event);" onKeyUp="return mascaraMoeda(this,event);" onblur="atualizaValoresProposta()" maxlength="19" /></td>
			  			</tr>
			  			<tr>
                      ...
                      <?php if($aProposta["tf_ppst"]!="S"){?>
			       	    <img name="btAprovarImov" id="btAprovarImov" src="images/buttons/bt_aprovar_sim.gif" alt="Aprovar Im?vel"  class="im" onClick="aprovarImovel('<?=$crypt->encrypt('aprovarImovel');?>');" style="visibility:hidden;" />
                        <?php }else{?>
                        <img name="btAprovarImov" id="btAprovarImov" src="images/buttons/bt_aprovar_sim.gif" alt="Aprovar Im?vel"  class="im" onClick="aprovarImovel2('<?=$crypt->encrypt('aprovarImovel');?>');" style="visibility:hidden;" />
			       	    <? } /*depois de voltar a resposta da previ verificar se ? preciso
			       	    exibir botao "cancelar aprova??o" que ir? limpar o campo da
			       	    "data de aprova??o" e deixar? o form imovel aberto*/ ?>
			          </td>
			  			</tr>
			  		</table>
			  	</div>
			...
			echo qd_exigencia('imov_exigencia','imov_BtExig','imov_addexig', 'imov_btsalvarexigencia',$_POST['imov_addexig'],$aProposta["cod_ppst"],'fimexigimov');}
			 include('bl_ckls_imovel.inc.php'); ?></div>
				...
				<? if($aProposta["imovel"]["dtavaliacao_imov"]){ ?>
				  <div class="grupoDados" style="width:475px;">
						...
						<tr>
                          ...
                          <input type="hidden" name="vlavaliacao_imov" id="vlavaliacao_imov" value="<?=$utils->formataMoeda($aProposta["imovel"]["vlavaliacao_imov"]);?>"  maxlength="19"   />
						 ...
						 if( $cLOGIN->iLEVEL_USUA == TPUSER_ATENDENTE){
						 ?>
						 <img name="btCancImov" id="btCancImov" src="images/buttons/bt_cancelar.gif" alt="Cancelar Aprova??o de Im?vel"  class="im" onClick="cancImovel('<?=$crypt->encrypt('CancImovel');?>')" />   
