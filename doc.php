...
function proxMes($mes)	
{
	switch ($mes){
	
	case 1: $mes = mes+1; break;
	case 2: $mes = $mes+1; break;
	case 3: $mes = $mes+1; break;
	case 4: $mes = $mes+1; break;
	case 5: $mes = $mes+1; break;
	case 6: $mes = $mes+1; break;
	case 7: $mes = $mes+1; break;
	case 8: $mes = $mes+1; break;
	case 9: $mes = $mes+1; break;
	case 10: $mes = $mes+1; break;
	case 11: $mes = $mes+1; break;
...
if($tipo_ppst=='S'){ // Qualificação do Participante de Proposta Simples
	...
	//______________________ Dados de Devedor Solidário __________________
	...
	if($flg_crd=='S'){
		...
		$credor="";
		$desc_crd="<font color='#FFFFFF'>LN</font><br>
	...
	// ____________Dados de Valores para Quadro III _____________________________________
	...
	// ____________Dados de Valores para Quadro IV ______________________________________
	// Valor FGTS
	...
	//Qualificação de Contas do Devedor
	...
	if($prop_cidade==4076)
	{
		$infofixa="Foram apresentadas em nome do Vendedor as certidões exigidas pela Lei 7.433, de 18/12/85, quais sejam: Certidões dos 1°, 2°, 3°, 4° e 9° Ofícios de Distribuição, 1° e 2° Ofícios de Interdições e Tutelas, situação enfitêutica, quitação fiscal, 9° Ofício – Executivos Fiscais e declaração de quitação condominial relativas ao imóvel. O imóvel objeto do presente contrato não é foreiro, conforme certidão expedida pela Prefeitura do Município do Rio de Janeiro. Quando for a hipótese de imóvel foreiro, a certidão apresentada, bem como o recolhimento de laudêmio encontrar-se-á mencionado no campo Informações Adicionais.";
	}elseif($prop_cidade==3172)
	{
		$infofixa="Declara o Vendedor que não existem quaisquer débitos que recaiam sobre  o imóvel objeto deste contrato, conforme Certidão Negativa de Tributos Municipais, expedida pela Prefeitura Municipal de Niterói. Foram apresentadas as certidões exigidas pela Lei 7.433, de 18/12/85, relativamente a Interdição, Tutela e Curatela, expedida pelo Registro Civil da Primeira Zona Judiciária de Niterói, 1° Distribuidor de Niterói e Justiça Federal, em nome do Vendedor.";
	}elseif($prop_cidade==1445)
	{
		$infofixa="Na conformidade da Lei n° 7.433, de 18/12/1985, do Decreto n° 93.240, de 09/09/1986, da Corregedoria da Justiça do Estado do Paraná, foram apresentadas as Certidões exigidas, inclusive a de feitos ajuizados, do conhecimento das partes. Declara ainda o outorgante não existirem ações reais e pessoais reipersecutórias relativas ao imóvel objeto da presente Escritura, e de outros ônus reais incidentes sobre o mesmo, conforme Decreto n° 93.240, de 09/09/86, Art. 1°, inc. V, parágrafo 3°";
	...
	{
		$dev1="doravante denominados simplesmente, em conjunto, os DEVEDORES";
		$dev2="os DEVEDORES";
		$dev3="dos DEVEDORES";
		$dev4="aos DEVEDORES";
		$dev5="pelos DEVEDORES";
		$dev6="Os DEVEDORES";
		$assinatura_dev="
			<table border='0'>
				<tr>
		...
		if($prop_sex=='M'){
			$dev1="doravante denominado simplesmente, o DEVEDOR";
			$dev2="o DEVEDOR";
			$dev3="do DEVEDOR";
			$dev4="ao DEVEDOR";
			$dev5="pelo DEVEDOR";
		...
		if($prop_sex=='F'){
			$dev1="doravante denominada simplesmente, a DEVEDORA";
			$dev2="a DEVEDORA";
			$dev3="da DEVEDORA";
			$dev4="à DEVEDORA";
			$dev5="pela DEVEDORA";
	...
	if($prop_sex=='M')
	{
		if($prop_civil==2)//EST. CIVIL CASADO
		{
			if($prop_reg==1)// Comunhão Parcial de Bens antes da lei
			{
				if($prop_nacional==$conj_nacional)
				{
									$emitente="<b>".maiusculo($prop_nome)." e s/m ".maiusculo($conj_nome)."</b>, ".$nacional.", casados  pelo regime de Comunhão Parcial de Bens,  em ".$prop_dtcasamento.", conforme escritura de pacto antenupcial lavrada no ".$prop_loclavrado.", no Livro ".$prop_livro.", Folhas ".$prop_folha.", em ".$prop_dtlavrado.", registrada sob o nr. ".$prop_numregistro." ".$habens.", ele bancário, ela ".$conj_profissao.", portadores das carteiras  de identidade nrs. ".$prop_numdoc." e ".$conj_numdoc.", expedidas por ".maiusculo($prop_emissor)."/".maiusculo($conj_emissor).", respectivamente, inscritos no CPFMF sob os nrs. ".$prop_cpf." e ".$conj_cpf.", residentes e domiciliados no(a) ".$endereco.", ".$dev1.$procurador;
				}else
				{
			...
			if($prop_reg==7)//Comunhão Parcial de Bens depois da lei
			{
				if($prop_nacional==$conj_nacional)
				{
									$emitente="<b>".maiusculo($prop_nome)." e s/m ".maiusculo($conj_nome)."</b>, ".$nacional.", casados  pelo regime de Comunhão Parcial de Bens,  em ".$prop_dtcasamento.", ele bancário, ela ".$conj_profissao.", portadores das carteiras  de identidade nrs. ".$prop_numdoc." e ".$conj_numdoc.", expedidas por ".maiusculo($prop_emissor)."/".maiusculo($conj_emissor).", respectivamente, inscritos no CPFMF sob os nrs. ".$prop_cpf." e ".$conj_cpf.", residentes e domiciliados no(a) ".$endereco.", ".$dev1.$procurador;
				}else
				{
			...
			if($prop_reg==2)// Comunhão Universal de Bens antes da lei
			{
					if($prop_nacional==$conj_nacional)
					{
										$emitente="<b>".maiusculo($prop_nome)." e s/m ".maiusculo($conj_nome)."</b>, ".$nacional.", casados pelo regime de Comunhão Universal de Bens,  em ".$prop_dtcasamento.", ele bancário, ela ".$conj_profissao.", portadores das carteiras  de identidade nrs. ".$prop_numdoc." e ".$conj_numdoc.", expedidas por ".maiusculo($prop_emissor)."/".maiusculo($conj_emissor).", respectivamente, inscritos no CPFMF sob os nrs. ".$prop_cpf." e ".$conj_cpf.", residentes e domiciliados no(a) ".$endereco.", ".$dev1.$procurador;
					}else
					{
										$emitente="<b>".maiusculo($prop_nome)." e s/m ".maiusculo($conj_nome)."</b>, ele ".$emit_nacional.", ela ".$conj_nacional.", casados pelo regime de Comunhão Universal de Bens,  em ".$prop_dtcasamento.", ele bancário, ela ".$conj_profissao.", portadores das carteiras  de identidade nrs. ".$prop_numdoc." e ".$conj_numdoc.", expedidas por ".maiusculo($prop_emissor)."/".maiusculo($conj_emissor).", respectivamente, inscritos no CPFMF sob os nrs. ".$prop_cpf." e ".$conj_cpf.", residentes e domiciliados no(a) ".$endereco.", ".$dev1.$procurador;
					}
			}
		...
		if($prop_civil==3) $estciv='separado';
		if($prop_civil==4) $estciv='divorciado';
		if($prop_civil==5) $estciv='viúvo';
					
		if($conj_civil==1) $pc_estciv='solteira';
		elseif($conj_civil==3) $pc_estciv='separada';
		elseif($conj_civil==4) $pc_estciv='divorciada';
		elseif($conj_civil==5) $pc_estciv='viúva';

		if($prop_civil!=2 && $flguniest=='S')
		{
				$emitente="<b>".maiusculo($prop_nome)."</b>, ".$estciv.", convivendo em união estável, nos termos da Lei nº. 9.278/96 e alterações do art. 1.723 do Código Civil Brasileiro com <b>".$conj_nome."</b>, ".$pc_estciv.", ele bancário, ela ".$conj_profissao.", portadores das carteiras  de identidade nrs. ".$prop_numdoc." e ".$conj_numdoc.", expedidas por ".maiusculo($prop_emissor)."/".maiusculo($conj_emissor).", respectivamente, inscritos no CPFMF sob os nrs. ".$prop_cpf." e ".$conj_cpf.", residentes e domiciliados no(a) ".$endereco.", ".$dev1.$procurador;
		
		}elseif($prop_civil!=2 && ($flguniest=='N' || $flguniest==''))
		{
				$emitente="<b>".maiusculo($prop_nome)."</b>, ".$emit_nacional.", ".$estciv.", portador da carteira de identidade nr. ".$prop_numdoc.", expedida por ".$prop_emissor.", inscrito no CPFMF sob o nr. ".$prop_cpf.", residente e domiciliado no(a) ".$endereco.", ".$dev1.$procurador;
		}
	}//fim if ($prop_sex=='M')
	...
	if($prop_sex=='F')
	{
		if($prop_civil==2)//EST. CIVIL CASADO
		{
			if($prop_reg==1)//Comunhão Parcial de Bens antes da lei
			{
				if($prop_nacional==$conj_nacional)
				{
									$emitente="<b>".maiusculo($prop_nome)." e s/m ".maiusculo($conj_nome)."</b>, ".$nacional.", casados  pelo regime de Comunhão Parcial de Bens,  em ".$prop_dtcasamento.", conforme escritura de pacto antenupcial lavrada no ".$prop_loclavrado.", no Livro ".$prop_livro.", Folhas ".$prop_folha.", em ".$prop_dtlavrado.", registrada sob o nr. ".$prop_numregistro." ".$habens.", ela bancária, ele ".$conj_profissao.", portadores das carteiras  de identidade nrs. ".$prop_numdoc." e ".$conj_numdoc.", expedidas por ".maiusculo($prop_emissor)."/".maiusculo($conj_emissor).", respectivamente, inscritos no CPFMF sob os nrs. ".$prop_cpf." e ".$conj_cpf.", residentes e domiciliados no(a) ".$endereco." <br>".$procurador;
				}else
				{
			...
			if($prop_reg==7)//Comunhão Parcial de Bens depois da lei
			{
				if($prop_nacional==$conj_nacional)
				{
									$emitente="<b>".maiusculo($prop_nome)." e s/m ".maiusculo($conj_nome)."</b>, ".$nacional.", casados  pelo regime de Comunhão Parcial de Bens,  em ".$prop_dtcasamento.", ela bancária, ele ".$conj_profissao.", portadores das carteiras  de identidade nrs. ".$prop_numdoc." e ".$conj_numdoc.", expedidas por ".maiusculo($prop_emissor)."/".maiusculo($conj_emissor).", respectivamente, inscritos no CPFMF sob os nrs. ".$prop_cpf." e ".$conj_cpf.", residentes e domiciliados no(a) ".$endereco.", ".$dev1.$procurador;
				}else
				{
			...
			if($prop_reg==2)//Comunhão Universal de Bens antes da lei
			{
					...
					{
										$emitente="<b>".maiusculo($prop_nome)." e s/m ".maiusculo($conj_nome)."</b>, ela ".$emit_nacional.", ele ".$conj_nacional.", casados pelo regime de Comunhão Universal de Bens,  em ".$prop_dtcasamento.", ela bancária, ele ".$conj_profissao.", portadores das carteiras  de identidade nrs. ".$prop_numdoc." e ".$conj_numdoc.", expedidas por ".maiusculo($prop_emissor)."/".maiusculo($conj_emissor).", respectivamente, inscritos no CPFMF sob os nrs. ".$prop_cpf." e ".$conj_cpf.", residentes e domiciliados no(a) ".$endereco.", ".$dev1.$procurador;
					}
			}
			if($prop_reg==3)//Comunhão Universal de Bens depois da lei
			{
				if($prop_nacional==$conj_nacional)
				{
									$emitente="<b>".maiusculo($prop_nome)." e s/m ".maiusculo($conj_nome)."</b>, ".$nacional." , casados pelo regime de Comunhão Universal de Bens,  em ".$prop_dtcasamento.", conforme escritura de pacto antenupcial lavrada no ".$prop_loclavrado.", no Livro ".$prop_livro.", Folhas ".$prop_folha.", em ".$prop_dtlavrado.", registrada sob o nr. ".$prop_numregistro." ".$habens.", ela bancária, ele ".$conj_profissao.", portadores das carteiras  de identidade nrs. ".$prop_numdoc." e ".$conj_numdoc.", expedidas por ".maiusculo($prop_emissor)."/".maiusculo($conj_emissor).", respectivamente, inscritos no CPFMF sob os nrs. ".$prop_cpf." e ".$conj_cpf.", residentes e domiciliados no(a) ".$endereco.", ".$dev1.$procurador;
				}else
				{
									$emitente="<b>".maiusculo($prop_nome)." e s/m ".maiusculo($conj_nome)."</b>, ela ".$emit_nacional.", ele ".$conj_nacional.", casados pelo regime de Comunhão Universal de Bens,  em ".$prop_dtcasamento.", conforme escritura de pacto antenupcial lavrada no ".$prop_loclavrado.", no Livro ".$prop_livro.", Folhas ".$prop_folha.", em ".$prop_dtlavrado.", registrada sob o nr. ".$prop_numregistro." ".$habens.", ela bancária, ele ".$conj_profissao.", portadores das carteiras  de identidade nrs. ".$prop_numdoc." e ".$conj_numdoc.", expedidas por ".maiusculo($prop_emissor)."/".maiusculo($conj_emissor).", respectivamente, inscritos no CPFMF sob os nrs. ".$prop_cpf." e ".$conj_cpf.", residentes e domiciliados no(a) ".$endereco.", ".$dev1.$procurador;
				}
		...
		if($prop_civil==3) $estciv='separada';
		if($prop_civil==4) $estciv='divorciada';
		if($prop_civil==5) $estciv='viúva';

		if($conj_civil==1) $pc_estciv='solteiro';
		elseif($conj_civil==3) $pc_estciv='separado';
		elseif($conj_civil==4) $pc_estciv='divorciado';
		elseif($conj_civil==5) $pc_estciv='viúvo';

		if($prop_civil!=2 && $flguniest=='S')
		{
				$emitente="<b>".maiusculo($prop_nome)."</b>, ".$estciv.", convivendo em união estável, nos termos da Lei nº. 9.278/96 e alterações do art. 1.723 do Código Civil Brasileiro com <b>".$conj_nome."</b>, ".$pc_estciv.", ela bancária, ele ".$conj_profissao.", portadores das carteiras  de identidade nrs. ".$prop_numdoc." e ".$conj_numdoc.", expedidas por ".maiusculo($prop_emissor)."/".maiusculo($conj_emissor).", respectivamente, inscritos no CPFMF sob os nrs. ".$prop_cpf." e ".$conj_cpf.", residentes e domiciliados no(a) ".$endereco.", ".$dev1.$procurador;
		
		}elseif($prop_civil!=2 && ($flguniest=='N' || $flguniest==''))
		{
				$emitente="<b>".maiusculo($prop_nome)."</b>, ".$emit_nacional.", ".$estciv.", portadora da carteira de identidade nr. ".$prop_numdoc.", expedida por ".$prop_emissor.", inscrita no CPFMF sob o nr. ".$prop_cpf.", residente e domiciliada no(a) ".$endereco.", ".$dev1.$procurador;
		}
...
#################################### QUALIFICAÇÃO DE VENDEDOR(ES) ##################################################
	...
	if($flgproc_v=='S')
	{
		$procurador_v=", neste ato representado(s) por seu(sua) bastante procurador(a) ".$reg['PROC_VPROC'].".";
		$ass_proc="P.P. ";
	}else
	{
		$procurador_v=".";
	...
	while($registro = mysql_fetch_array($result, MYSQL_ASSOC))
	{
		...
		if($vend_tipo[$a]==1){
			...
			$regvf = mysql_fetch_array($rsvf, MYSQL_ASSOC);
				...
				$vendf_profissao[$a]	= $regvf['COD_PROF'];
			...
			//§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§
			#__________________________ QUALIFICAÇÃO VENDEDOR(ES) PESSOA FÍSICA _________________________#
			//§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§
					if($a==1 && $linhas_v>1)
					{
						$vendedor .="<b>".$a."-</b> ";
					...
					if($vendf_sex[$a]=='M')// Vendedor PF Masculino
					{
						if($vendf_estciv[$a]==2)//EST. CIVIL CASADO
						{
				...
				
							{
								if($vendf_nacional[$a]==$vconj_nacional[$a])
								{
													$vendedor .="<b>".maiusculo($vend_nome[$a])." e s/m ".maiusculo($vconj_nome[$a])."</b>, ".$vnacional[$a]." ,casados pelo regime de Comunhão Parcial de Bens,  em ".$vconj_dtcasamento[$a].", conforme escritura de pacto antenupcial lavrada no ".$vcpacto_loc[$a].", no Livro ".$vcpacto_livro[$a].", Folhas ".$vcpacto_folha[$a].", em ".$vcpacto_data[$a].", registrada sob o nr. ".$vcpacto_reg[$a].", ele ".$v_profissao[$a].", ela ".$vconj_cargoemp[$a].", portadores das carteiras  de identidade nrs. ".$vendf_nrrg[$a]." e ".$vconj_nrrg[$a].", expedidas por ".maiusculo($vendf_orgrg[$a])."/".maiusculo($vconj_orgrg[$a]).", respectivamente, inscritos no CPFMF sob os nrs. ".$vendf_cpf[$a]." e ".$vconj_cpf[$a].", residentes e domiciliados no(a) ".$v_endereco[$a].$limite_v;
								}else
								{
							...
							if($vconj_regime[$a]==7)//Comunhão Parcial de Bens depois da lei
							{
								if($vendf_nacional[$a]==$vconj_nacional[$a])
								{
													$vendedor .="<b>".maiusculo($vend_nome[$a])." e s/m ".maiusculo($vconj_nome[$a])."</b>, ".$vnacional[$a]." ,casados pelo regime de Comunhão Parcial de Bens,  em ".$vconj_dtcasamento[$a].", ele ".$v_profissao[$a].", ela ".$vconj_cargoemp[$a].", portadores das carteiras  de identidade nrs. ".$vendf_nrrg[$a]." e ".$vconj_nrrg[$a].", expedidas por ".maiusculo($vendf_orgrg[$a])."/".maiusculo($vconj_orgrg[$a]).", respectivamente, inscritos no CPFMF sob os nrs. ".$vendf_cpf[$a]." e ".$vconj_cpf[$a].", residentes e domiciliados no(a) ".$v_endereco[$a].$limite_v;
								}else
								{
													$vendedor .="<b>".maiusculo($vend_nome[$a])." e s/m ".maiusculo($vconj_nome[$a])."</b>, ele ".$v_nacional[$a].", ela ".$vc_nacional[$a].", casados pelo regime de Comunhão Parcial de Bens,  em ".$vconj_dtcasamento[$a].", ele ".$v_profissao[$a].", ela ".$vconj_cargoemp[$a].", portadores das carteiras  de identidade nrs. ".$vendf_nrrg[$a]." e ".$vconj_nrrg[$a].", expedidas por ".maiusculo($vendf_orgrg[$a])."/".maiusculo($vconj_orgrg[$a]).", respectivamente, inscritos no CPFMF sob os nrs. ".$vendf_cpf[$a]." e ".$vconj_cpf[$a].", residentes e domiciliados no(a) ".$v_endereco[$a].$limite_v;
								}
							...
							if($vconj_regime[$a]==2)//Comunhão Universal de Bens antes da lei
							{
								if($vendf_nacional[$a]==$vconj_nacional[$a])
								{
													$vendedor .="<b>".maiusculo($vend_nome[$a])." e s/m ".maiusculo($vconj_nome[$a])."</b>, ".$vnacional[$a].", casados pelo regime de Comunhão Universal de Bens,  em ".$vconj_dtcasamento[$a].", ele ".$v_profissao[$a].", ela ".$vconj_cargoemp[$a].", portadores das carteiras  de identidade nrs. ".$vendf_nrrg[$a]." e ".$vconj_nrrg[$a].", expedidas por ".maiusculo($vendf_orgrg[$a])."/".maiusculo($vconj_orgrg[$a]).", respectivamente, inscritos no CPFMF sob os nrs. ".$vendf_cpf[$a]." e ".$vconj_cpf[$a].", residentes e domiciliados no(a) ".$v_endereco[$a].$limite_v;
								}else
								{
													$vendedor .="<b>".maiusculo($vend_nome[$a])." e s/m ".maiusculo($vconj_nome[$a])."</b>, ele ".$vendf_nacional[$a].", ela ".$vconj_nacional[$a].", casados pelo regime de Comunhão Universal de Bens,  em ".$vconj_dtcasamento[$a].", ele ".$v_profissao[$a].", ela ".$vconj_cargoemp[$a].", portadores das carteiras  de identidade nrs. ".$vendf_nrrg[$a]." e ".$vconj_nrrg[$a].", expedidas por ".maiusculo($vendf_orgrg[$a])."/".maiusculo($vconj_orgrg[$a]).", respectivamente, inscritos no CPFMF sob os nrs. ".$vendf_cpf[$a]." e ".$vconj_cpf[$a].", residentes e domiciliados no(a) ".$v_endereco[$a].$limite_v;
								}
							}
							if($vconj_regime[$a]==3)//Comunhão Universal de Bens depois da lei
							{
								if($vendf_nacional[$a]==$vconj_nacional[$a])
								{
													$vendedor .="<b>".maiusculo($vend_nome[$a])." e s/m ".maiusculo($vconj_nome[$a])."</b>, ".$vnacional[$a].", casados pelo regime de Comunhão Universal de Bens,  em ".$vconj_dtcasamento[$a].", conforme escritura de pacto antenupcial lavrada no ".$vcpacto_loc[$a].", no Livro ".$vcpacto_livro[$a].", Folhas ".$vcpacto_folha[$a].", em ".$vcpacto_data[$a].", registrada sob o nr. ".$vcpacto_reg[$a].", ele ".$v_profissao[$a].", ela ".$vconj_cargoemp[$a].", portadores das carteiras  de identidade nrs. ".$vendf_nrrg[$a]." e ".$vconj_nrrg[$a].", expedidas por ".maiusculo($vendf_orgrg[$a])."/".maiusculo($vconj_orgrg[$a]).", respectivamente, inscritos no CPFMF sob os nrs. ".$vendf_cpf[$a]." e ".$vconj_cpf[$a].", residentes e domiciliados no(a) ".$v_endereco[$a].$limite_v;
								}else
								{
													$vendedor .="<b>".maiusculo($vend_nome[$a])." e s/m ".maiusculo($vconj_nome[$a])."</b>, ele ".$vendf_nacional[$a].", ela ".$vconj_nacional[$a].", casados pelo regime de Comunhão Universal de Bens,  em ".$vconj_dtcasamento[$a].", conforme escritura de pacto antenupcial lavrada no ".$vcpacto_loc[$a].", no Livro ".$vcpacto_livro[$a].", Folhas ".$vcpacto_folha[$a].", em ".$vcpacto_data[$a].", registrada sob o nr. ".$vcpacto_reg[$a].", ele ".$v_profissao[$a].", ela ".$vconj_cargoemp[$a].", portadores das carteiras  de identidade nrs. ".$vendf_nrrg[$a]." e ".$vconj_nrrg[$a].", expedidas por ".maiusculo($vendf_orgrg[$a])."/".maiusculo($vconj_orgrg[$a]).", respectivamente, inscritos no CPFMF sob os nrs. ".$vendf_cpf[$a]." e ".$vconj_cpf[$a].", residentes e domiciliados no(a) ".$v_endereco[$a].$limite_v;
								}
							}
							...
							if($vconj_regime[$a]==6)//Separação de Bens obrigatórioa
							{
								if($vendf_nacional[$a]==$vconj_nacional[$a])
								{
													$vendedor .="<b>".maiusculo($vend_nome[$a])." e s/m ".maiusculo($vconj_nome[$a])."</b>, ".$vnacional[$a]." ,casados pelo regime de Separação Obrigatória de bens, nos termos do artigo 1641 do Código Civil Brasileiro,  em ".$vconj_dtcasamento[$a].", ele ".$v_profissao[$a].", ela ".$vconj_cargoemp[$a].", portadores das carteiras  de identidade nrs. ".$vendf_nrrg[$a]." e ".$vconj_nrrg[$a].", expedidas por ".maiusculo($vendf_orgrg[$a])."/".maiusculo($vconj_orgrg[$a]).", respectivamente, inscritos no CPFMF sob os nrs. ".$vendf_cpf[$a]." e ".$vconj_cpf[$a].", residentes e domiciliados no(a) ".$v_endereco[$a].$limite_v;
								}else
								{
					...
					elseif($vendf_estciv[$a]==3) $v_estciv[$a]='separado';
					...
					elseif($vconj_civil[$a]==3) $vcj_estciv[$a]='separada';
					...
					{
							$vendedor .="<b>".maiusculo($vend_nome[$a])."</b>, ".$v_estciv[$a].", convivendo em união estável, nos termos da Lei nº. 9.278/96 e alterações do art. 1.723 do Código Civil Brasileiro com ".$vconj_nome[$a].", ".$vcj_estciv[$a].", brasileiros, portadores das carteiras  de identidade nrs. ".$vendf_nrrg[$a]." e ".$vconj_nrrg[$a].", expedidas por ".maiusculo($vendf_orgrg[$a])."/".maiusculo($vconj_orgrg[$a]).", respectivamente, inscritos no CPFMF sob os nrs. ".$vendf_cpf[$a]." e ".$vconj_cpf[$a].", residentes e domiciliados no(a) ".$v_endereco[$a].$limite_v;
					
					}elseif($vendf_estciv[$a]!=2 && ($vendf_flguniest[$a]=='N' || $vendf_flguniest[$a]==''))
					{
							$vendedor .="<b>".maiusculo($vend_nome[$a])."</b>, ".$v_nacional[$a].", ".$v_estciv[$a].", portador da carteira de identidade nr. ".$vendf_nrrg[$a].", expedida por ".$vendf_orgrg[$a].", inscrito no CPFMF sob o nr. ".$vendf_cpf[$a].", residente e domiciliado no(a) ".$v_endereco[$a].$limite_v;
					}
					...
					if($vendf_sex[$a]=='F')// Vendedor PF Feminino
					{
						if($vendf_estciv[$a]==2)//EST. CIVIL CASADO
						{
							...
							if($vconj_regime[$a]==7)//Comunhão Parcial de Bens depois da lei
							{
								if($vendf_nacional[$a]==$vconj_nacional[$a])
								{
													$vendedor .="<b>".maiusculo($vend_nome[$a])." e s/m ".maiusculo($vconj_nome[$a])."</b>, ".$vnacional[$a]." ,casados pelo regime de Comunhão Parcial de Bens,  em ".$vconj_dtcasamento[$a].", ela ".$v_profissao[$a].", ele ".$vconj_cargoemp[$a].", portadores das carteiras  de identidade nrs. ".$vendf_nrrg[$a]." e ".$vconj_nrrg[$a].", expedidas por ".maiusculo($vendf_orgrg[$a])."/".maiusculo($vconj_orgrg[$a]).", respectivamente, inscritos no CPFMF sob os nrs. ".$vendf_cpf[$a]." e ".$vconj_cpf[$a].", residentes e domiciliados no(a) ".$v_endereco[$a].$limite_v;
								}else
								{
													$vendedor .="<b>".maiusculo($vend_nome[$a])." e s/m ".maiusculo($vconj_nome[$a])."</b>, ela ".$v_nacional[$a].", ele ".$vc_nacional[$a].", casados pelo regime de Comunhão Parcial de Bens,  em ".$vconj_dtcasamento[$a].", ela ".$v_profissao[$a].", ele ".$vconj_cargoemp[$a].", portadores das carteiras  de identidade nrs. ".$vendf_nrrg[$a]." e ".$vconj_nrrg[$a].", expedidas por ".maiusculo($vendf_orgrg[$a])."/".maiusculo($vconj_orgrg[$a]).", respectivamente, inscritos no CPFMF sob os nrs. ".$vendf_cpf[$a]." e ".$vconj_cpf[$a].", residentes e domiciliados no(a) ".$v_endereco[$a].$limite_v;
								}
							...
							if($vconj_regime[$a]==2)//Comunhão Universal de Bens antes da lei
							{
								if($vendf_nacional[$a]==$vconj_nacional[$a])
								{
													$vendedor .="<b>".maiusculo($vend_nome[$a])." e s/m ".maiusculo($vconj_nome[$a])."</b>, ".$vnacional[$a].", casados pelo regime de Comunhão Universal de Bens,  em ".$vconj_dtcasamento[$a].", ela ".$v_profissao[$a].", ele ".$vconj_cargoemp[$a].", portadores das carteiras  de identidade nrs. ".$vendf_nrrg[$a]." e ".$vconj_nrrg[$a].", expedidas por ".maiusculo($vendf_orgrg[$a])."/".maiusculo($vconj_orgrg[$a]).", respectivamente, inscritos no CPFMF sob os nrs. ".$vendf_cpf[$a]." e ".$vconj_cpf[$a].", residentes e domiciliados no(a) ".$v_endereco[$a].$limite_v;
								}else
								{
													$vendedor .="<b>".maiusculo($vend_nome[$a])." e s/m ".maiusculo($vconj_nome[$a])."</b>, ela ".$vendf_nacional[$a].", ele ".$vconj_nacional[$a].", casados pelo regime de Comunhão Universal de Bens,  em ".$vconj_dtcasamento[$a].", ela ".$v_profissao[$a].", ele ".$vconj_cargoemp[$a].", portadores das carteiras  de identidade nrs. ".$vendf_nrrg[$a]." e ".$vconj_nrrg[$a].", expedidas por ".maiusculo($vendf_orgrg[$a])."/".maiusculo($vconj_orgrg[$a]).", respectivamente, inscritos no CPFMF sob os nrs. ".$vendf_cpf[$a]." e ".$vconj_cpf[$a].", residentes e domiciliados no(a) ".$v_endereco[$a].$limite_v;
								}
							}
							if($vconj_regime[$a]==3)//Comunhão Universal de Bens depois da lei
							{
								if($vendf_nacional[$a]==$vconj_nacional[$a])
								{
													$vendedor .="<b>".maiusculo($vend_nome[$a])." e s/m ".maiusculo($vconj_nome[$a])."</b>, ".$vnacional[$a].", casados pelo regime de Comunhão Universal de Bens,  em ".$vconj_dtcasamento[$a].", conforme escritura de pacto antenupcial lavrada no ".$vcpacto_loc[$a].", no Livro ".$vcpacto_livro[$a].", Folhas ".$vcpacto_folha[$a].", em ".$vcpacto_data[$a].", registrada sob o nr. ".$vcpacto_reg[$a].", ela ".$v_profissao[$a].", ele ".$vconj_cargoemp[$a].", portadores das carteiras  de identidade nrs. ".$vendf_nrrg[$a]." e ".$vconj_nrrg[$a].", expedidas por ".maiusculo($vendf_orgrg[$a])."/".maiusculo($vconj_orgrg[$a]).", respectivamente, inscritos no CPFMF sob os nrs. ".$vendf_cpf[$a]." e ".$vconj_cpf[$a].", residentes e domiciliados no(a) ".$v_endereco[$a].$limite_v;
								}else
								{
													$vendedor .="<b>".maiusculo($vend_nome[$a])." e s/m ".maiusculo($vconj_nome[$a])."</b>, ela ".$vendf_nacional[$a].", ele ".$vconj_nacional[$a].", casados pelo regime de Comunhão Universal de Bens,  em ".$vconj_dtcasamento[$a].", conforme escritura de pacto antenupcial lavrada no ".$vcpacto_loc[$a].", no Livro ".$vcpacto_livro[$a].", Folhas ".$vcpacto_folha[$a].", em ".$vcpacto_data[$a].", registrada sob o nr. ".$vcpacto_reg[$a].", ela ".$v_profissao[$a].", ele ".$vconj_cargoemp[$a].", portadores das carteiras  de identidade nrs. ".$vendf_nrrg[$a]." e ".$vconj_nrrg[$a].", expedidas por ".maiusculo($vendf_orgrg[$a])."/".maiusculo($vconj_orgrg[$a]).", respectivamente, inscritos no CPFMF sob os nrs. ".$vendf_cpf[$a]." e ".$vconj_cpf[$a].", residentes e domiciliados no(a) ".$v_endereco[$a].$limite_v;
								}
							}
							if($vconj_regime[$a]==5)//Separação de Bens com pacto
							{
							...
							if($vconj_regime[$a]==6)//Separação de Bens obrigatórioa
							{
								if($vendf_nacional[$a]==$vconj_nacional[$a])
								{
													$vendedor .="<b>".maiusculo($vend_nome[$a])." e s/m ".maiusculo($vconj_nome[$a])."</b>, ".$vnacional[$a]." ,casados pelo regime de Separação Obrigatória de bens, nos termos do artigo 1641 do Código Civil Brasileiro,  em ".$vconj_dtcasamento[$a].", ela ".$v_profissao[$a].", ele ".$vconj_cargoemp[$a].", portadores das carteiras  de identidade nrs. ".$vendf_nrrg[$a]." e ".$vconj_nrrg[$a].", expedidas por ".maiusculo($vendf_orgrg[$a])."/".maiusculo($vconj_orgrg[$a]).", respectivamente, inscritos no CPFMF sob os nrs. ".$vendf_cpf[$a]." e ".$vconj_cpf[$a].", residentes e domiciliados no(a) ".$v_endereco[$a].$limite_v;
								}else
								{
					...
					elseif($vendf_estciv[$a]==3) $v_estciv[$a]='separada';
					...
					elseif($vconj_civil[$a]==3) $vcj_estciv[$a]='separado';
					elseif($vconj_civil[$a]==4) $vcj_estciv[$a]='divorciado';
					elseif($vconj_civil[$a]==5) $vcj_estciv[$a]='viúvo';
			
					if($vendf_estciv[$a]!=2 && $vendf_flguniest[$a]=='S')
					{
							$vendedor .="<b>".maiusculo($vend_nome[$a])."</b>, ".$v_estciv[$a].", convivendo em união estável, nos termos da Lei nº. 9.278/96 e alterações do art. 1.723 do Código Civil Brasileiro com ".$vconj_nome[$a].", ".$vcj_estciv[$a].", ela ".$v_profissao[$a].", ele ".$vconj_cargoemp[$a].", portadores das carteiras  de identidade nrs. ".$vendf_nrrg[$a]." e ".$vconj_nrrg[$a].", expedidas por ".maiusculo($vendf_orgrg[$a])."/".maiusculo($vconj_orgrg[$a]).", respectivamente, inscritos no CPFMF sob os nrs. ".$vendf_cpf[$a]." e ".$vconj_cpf[$a].", residentes e domiciliados no(a) ".$v_endereco[$a].$limite_v;
					
					}elseif($vendf_estciv[$a]!=2 && ($vendf_flguniest[$a]=='N' || $vendf_flguniest[$a]==''))
					{
							$vendedor .="<b>".maiusculo($vend_nome[$a])."</b>, ".$v_nacional[$a].", ".$v_estciv[$a].", portadora da carteira de identidade nr. ".$vendf_nrrg[$a].", expedida por ".$vendf_orgrg[$a].", inscrita no CPFMF sob o nr. ".$vendf_cpf[$a].", residente e domiciliada no(a) ".$v_endereco[$a].$limite_v;
					}
				}//Fim Vendedor PF Feminino
				if($linhas_v==$a){
					$vendedor .=" doravante denominado(s) VENDEDOR(ES)".$procurador_v;
				}
		...
		if($vend_tipo[$a]==2){
			...
			while($regvjs = mysql_fetch_array($rsvjs, MYSQL_ASSOC))
			{
				...
				if($vjsocio_sexo[$a][$b]=='M')
				{
					...
					elseif($vjsocio_estciv[$a][$b]==3) $vjs_estciv[$a][$b]='separado';
				...
				if($vjsocio_sexo[$a][$b]=='F')
				{
					...
					elseif($vjsocio_estciv[$a][$b]==3) $vjs_estciv[$a][$b]='separada';
			...
			//§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§
			#__________________________ QUALIFICAÇÃO VENDEDOR(ES) PESSOA JURÍDICA _________________________#
			//§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§§
			$vendedor .= "<b>".maiusculo($vend_nome[$a])."</b>, com sede e foro na(o) ".$v_endereco[$a].", inscrita no CNPJ sob nº. ".$vendj_cnpj[$a].", com seu Estatuto Social consolidado na ".$vendj_versaoestat[$a]." Alteração de Contrato Social, datada de ".$vendj_dtestatv[$a].", registrada na(o) ".$vendj_locestat[$a]." sob nº. ".$vendj_regestat[$a].", doravante denominado(a) simplesmente VENDEDOR(ES), representado por ".$ident." ".$socio.$procurador_v;
			$assinatura_vend .="
					<tr>
						<td width='600'></td>
						<td width='364' align='center'><b>________________________________________________</b></td>
...
//gerar em word
@header("Content-Disposition: attachment; filename=contrato_".$prop_nome.".doc");
//gerar em excel
//header("Content-Disposition: attachment; filename=usuario.xls");
// montando a tabela
		echo "<table border='1'  bordercolor='#000000'  cellspacing='0' width='600'>
					<tr>
		...
		echo "<b>PREÂMBULO</b><br><br>
		
					<b>QUADRO I – PARTES</b><br><br>
					
					COMPRADOR(ES)<br>					
					<table border='1'  bordercolor='#000000'  cellspacing='0' width='600'>
						<tr align='justify'>
							<td align='justify'>".$emitente."</td>
						</tr>
					</table><br>";
		echo "
					VENDEDOR(ES)<br>					
					<table border='1'  bordercolor='#000000'  cellspacing='0' width='600'>
						<tr>
							<td align='justify'>".$vendedor."<font color='#FFFFFF'>LN</font><br>
							    <font color='#FFFFFF'>LN</font></td>
						</tr>
					</table><br>
					FINANCIADOR".$credor."<br>					
					<table border='1'  bordercolor='#000000'  cellspacing='0' width='600'>
						<tr>
							<td align='justify'><b>CAIXA DE PREVIDÊNCIA DOS FUNCIONÁRIOS DO BANCO DO BRASIL </b>- entidade fechada de previdência complementar constituída na forma de sociedade civil sem fins lucrativos, inscrita no CNPJ/MF sob o n.º 33.754.482/0001-24 , com sede na cidade do Rio de Janeiro – RJ, doravante denominada PREVI, neste ato representada por sua bastante procuradora <b>ATHOS GESTÃO E SERVIÇOS LTDA.</b>, inscrita no CNPJ/MF sob o n° 00.839.032/001-85 - <b>Matriz</b>, com sede na Avenida Senador Xavier da Silva n° 294, São Francisco, Curitiba-PR e <b>Filial</b> na Rua Amália de Noronha n° 159, Jardim América, São Paulo-SP, inscrita no CNPJ/MF sob o n° 00.839.032/0002-66, neste ato representada por <b>JOSÉ AUGUSTO FERREIRA GOMES</b>, brasileiro, casado, analista de sistema, portador da cédula de identidade RG n° 4.840.773-SSP-SP, inscrito no CPF/MF sob o n° 576.730.988-49, com endereço comercial na Cidade de São Paulo-SP, na Rua Amália de Noronha, n° 159, Pinheiros, nos termos das procurações lavradas no 6° Oficio de Notas da Comarca do Rio de Janeiro - RJ, às fls. 085 do livro n° 6.474, datada de 08/10/2007 e no 1° Tabelionato de Notas de Curitiba - PR, às fls. 054 do livro 0721-P, datada de 07/01/2008, respectivamente, que são partes integrantes do presente contrato.</td>
 						</tr>
					</table><br>";
		echo "<b>
					QUADRO II – IMÓVEL OBJETO DESTE CONTRATO</b><br>					
					<table border='1'  bordercolor='#000000'  cellspacing='0' width='600'>
						<tr>
							<td align='justify'>".formataTexto($qualificacao_imov)."</td>
						</tr>
					</table><br>";
		echo "<b>
					QUADRO III – PREÇO DA COMPRA E VENDA</b><br>					
					<table border='1'  bordercolor='#000000'  cellspacing='0' width='600'>
						<tr>
		...
		echo "<b>
					QUADRO IV – ORIGEM DOS RECURSOS PARA PAGAMENTO DA COMPRA E VENDA</b><br>					
					<table border='1'  bordercolor='#000000'  cellspacing='0' width='600'>
						<tr>
						...
						<tr>
							<td>3. Recursos próprios de DEVEDOR a título de sinal a ser ressarcido pela PREVI e incluído no financiamento: R$ ".formataMoeda($prop_vlsinal)."</td>
						</tr>
						<tr>
		...
		echo "<b>
					QUADRO V – GRAVAMES EXISTENTES SOBRE O IMÓVEL</b><br>					
					<table border='1'  bordercolor='#000000'  cellspacing='0' width='600'>
						<tr>
		...
		echo "<b>
					QUADRO VI – FORMA DE LIBERAÇÃO DOS RECURSOS DO FINANCIAMENTO</b><br>					
					<table border='1'  bordercolor='#000000'  cellspacing='0' width='600'>
						<tr>
							<td>1. Recursos liberados ao VENDEDOR(ES) (somados recursos de FGTS e do FINANCIAMENTO): R$ ".formataMoeda($vlliberado-$prop_vlsinal)."</td>
						</tr>
						<tr>
						...
						<tr>
							<td>4. Crédito em conta-corrente:<br>
								4.1 VENDEDOR(ES): ".$vend_contas."
								4.2 COMPRADOR: ".$prop_contas."
								
						</tr>
					</table><br>";
		echo "<b>
					QUADRO VII – CONDIÇÕES DO FINANCIAMENTO</b><br>					
					<table border='1'  bordercolor='#000000'  cellspacing='0' width='600'>
						<tr>
						...
						<tr>
							<td>4. Taxa Efetiva Anual de Juros: 5,750 %<br>5. Taxa Efetiva Mensal de Juros: 0,467%</td>
						</tr>
						<tr>
						...
						<tr>
							<td>7. Data de Vencimento do primeiro pagamento mensal: 20/".proxMes(date('m'))."/".proxMesAno(date('m'))."</td>
						</tr>
						<tr>
		...
		echo "<b>
					QUADRO VIII – ENCARGOS MENSAIS</b><br>					
					<table border='1'  bordercolor='#000000'  cellspacing='0' width='600'>
						<tr>
		...
		echo "<b>
					QUADRO IV – SEGURO DE DANOS FÍSISCOS AO IMÓVEL</b><br>					
					<table border='1'  bordercolor='#000000'  cellspacing='0' width='600'>
						<tr>
							<td>1. Valor do primeiro prêmio anual: R$ ".formataMoeda($vl_premio)."</td>
						</tr>
						<tr>
							<td>2. Data do pagamento: 20/".proxMes(date('m'))."/".proxMesAno(date('m'))."</td>
						</tr>
					</table><br>";
		echo "<b>
					QUADRO X – RENDA BRUTA BASE DO(S) DEVEDOR(S)</b><br>					
					<table border='1'  bordercolor='#000000'  cellspacing='0' width='600'>
						<tr>
		...
		echo "<b>
					QUADRO XI – RESPONSABILIDADE DO(S) DEVEDOR(ES) NO PAGAMENTO DAS PRESTAÇÕES</b><br>					
					<table border='1'  bordercolor='#000000'  cellspacing='0' width='600'>
						<tr>
							<td width='546' align='center'>NOMES</td>
		...
		echo "<b>
					QUADRO XII – CLÁUSULAS ADICIONAIS</b><br>					
					<table border='1'  bordercolor='#000000'  cellspacing='0' width='600'>
						<tr>
							<td align='justify'>".$infofixa."<font color='#FFFFFF'>LN</font><br>
								<font color='#FFFFFF'>LN</font><br>
							    <font color='#FFFFFF'>LN</font><br>
								<font color='#FFFFFF'>LN</font><br>
								<font color='#FFFFFF'>LN</font><br>
		...
		echo "
				<br><p align='justify'>
										REÚNEM-SE as Partes, anteriormente nomeadas e qualificadas, para, de mútuo acordo, celebrar, por meio deste instrumento particular com força de escritura pública, nos termos do artigo 61, § 5º da Lei nº 4.380 de 21/08/64, com as alterações introduzidas pela Lei nº 5.049, de 29.06.66, o  presente CONTRATO DE COMPRA E VENDA DE IMÓVEL COM FINANCIAMENTO, GARANTIA DE ALIENAÇÃO FIDUCIÁRIA E OUTRAS AVENÇAS (“Contrato”), integrado para todos os efeitos de direito pelos Quadros que se encontram preenchidos no preâmbulo, garantido por alienação fiduciária de imóvel, constituída nos termos da Lei nº 9.514/97, e regido pelas cláusulas, termos e condições a seguir:</p>
				<p align='justify'><b>
										CLÁUSULA 1 – COMPRA E VENDA DO IMÓVEL</b></p>
				<p align='justify'>
										O VENDEDOR(ES), na qualidade de proprietário(s) e legítimo(s) possuidor(es) do imóvel descrito e caracterizado no Quadro II (“Imóvel”), pelo presente Contrato, vende e transfere ".$dev4.", que compra(m) e adquire(m) referido Imóvel, pelo preço certo e ajustado constante do Quadro III, na forma mencionada no Quadro VI, por meio dos recursos mencionados no Quadro IV.</p>
				<p align='justify'><b>
										Declarações do VENDEDOR(ES)</b></p>
			    <p align='justify'>
										1.1. O VENDEDOR(ES) declara(m) que: i) o Imóvel encontra-se livre e desembaraçado de quaisquer ônus, gravames ou restrições, pessoais ou reais, de qualquer origem, inclusive fiscal, judicial ou negocial extrajudicial, exceto os gravames descritos no Quadro V, os quais são liberados conforme previsto na cláusula 2 deste Contrato; ii) não existem, contra si ou contra qualquer dos antigos proprietários do Imóvel, ações reais ou pessoais reipersecutórias, conforme certidões expedidas pelo Registro de Imóveis competente; iii) sobre o Imóvel não pesam débitos fiscais, condominiais ou de contribuições devidas a associação que congregue os moradores do conjunto imobiliário a que pertence o Imóvel.</p>
				<p align='justify'>
										1.1.1. O VENDEDOR(ES) declara(m), ainda, que o seu estado civil é aquele que se encontra descrito no Quadro I. Caso viva em união estável, seu(sua) companheiro(a), qualificado(a) também no Quadro I, assina este Contrato, expressando sua integral anuência com a presente compra e venda, sem que tal concordância tenha qualquer reflexo de caráter registrário, pois não infringidos os princípios da especialidade subjetiva e da continuidade.</p>
				<p align='justify'><b>
										Quitação do preço do Imóvel</b></p>
				<p align='justify'>
										1.2. Considerando que receberá o pagamento do preço da presente compra e venda diretamente da PREVI, à exceção dos valores já pagos a título de sinal ".$dev5.", conforme descrito no Quadro IV, nos termos deste Contrato, o VENDEDOR(ES), neste ato, dá(dão) ".$dev4." a mais plena, geral, rasa e irrevogável quitação do preço do Imóvel, para dele nada mais reclamar com respeito a tal preço, podendo, tão somente, reclamar da PREVI a liberação dos recursos do Financiamento e do Fundo de Garantia por Tempo de Serviço - FGTS, na forma do Quadro VI, que ocorrerá conforme os termos e condições deste Contrato.</p>
				<p align='justify'><b>
										Transferência da posse sobre o Imóvel</b></p>
				<p align='justify'>						
										1.3. Em decorrência da compra e venda e da transferência do domínio sobre o Imóvel, efetuadas na forma do caput desta cláusula, o VENDEDOR(ES) cede(m) e transfere(m), neste ato, ".$dev4.", que os adquire, toda a posse que exerce sobre o Imóvel, bem como todos os direitos, pretensões e ações, inclusive possessórias, de sua titularidade, relativas ao Imóvel, para que ".$dev2." dele use, goze e livremente disponha, como proprietário exclusivo que passa a ser.</p>
				<p align='justify'><b>				
										Responsabilidade pela evicção</b></p>
				<p align='justify'>
										1.4. O VENDEDOR(ES) obriga(m)-se por si, seus herdeiros e sucessores, a qualquer título, a fazer esta venda sempre boa, firme e valiosa e a responder pela evicção de direito na forma da lei.</p>
				<p align='justify'><b>				
				...
				<p align='justify'>
										Caso, nesta data, exista gravame hipotecário ou de alienação fiduciária sobre o Imóvel, conforme mencionado no Quadro V, constituído em favor do CREDOR QUITANTE para garantir o cumprimento de obrigação do VENDEDOR(ES), este(s), neste ato, autoriza(m) expressamente a PREVI a entregar ao CREDOR QUITANTE, do montante do financiamento ora concedido, o valor mencionado no item 2 do Quadro VI, quitando as obrigações do VENDEDOR(ES) perante o CREDOR QUITANTE, e remindo, dessa forma, o gravame existente sobre o Imóvel. O montante entregue ao CREDOR QUITANTE será deduzido do valor a ser entregue ao VENDEDOR(ES) por força do financiamento ora concedido, conforme disposto na cláusula 3.2.</p>
				<p align='justify'><b>				
										Quitação das obrigações e liberação do gravame</b></p>
				<p align='justify'>						
										2.1. O CREDOR QUITANTE, concordando com a compra e venda do Imóvel ora contratada e recebendo da PREVI, neste ato, por meio de pagamento em cheque conforme descrito no Quadro VI, os recursos correspondentes ao pagamento de seu crédito, dá ao VENDEDOR(ES) a mais plena, rasa e irrevogável quitação com relação a tal dívida e autoriza expressamente o Sr. Oficial do Cartório de Registro de Imóveis competente a proceder o cancelamento do referido gravame existente sobre o Imóvel, desde que o faça concomitantemente com registro da propriedade fiduciária constituída em favor da PREVI nos termos da cláusula 13.</p>
				<p align='justify'><b>				
				...
				<p align='justify'>
										2.3. Além das condições estipuladas na cláusula 3.1, a liberação dos recursos referentes ao Financiamento conforme descrito nos itens 1 e 3 do Quadro VI encontra-se suspensivamente condicionada à comprovação à PREVI, por meio da entrega da ficha de matrícula atualizada do Imóvel, do efetivo cancelamento do gravame existente sobre o Imóvel em favor do CREDOR QUITANTE.</p>
				<p align='justify'><b>										
										CLÁUSULA 3 – FINANCIAMENTO</b></p>
				<p align='justify'>
										Para possibilitar o pagamento do preço do Imóvel, a PREVI, neste ato, concede ".$dev4." um financiamento, no valor total estipulado no Quadro VII (“Financiamento”), utilizando-se, para tanto, de recursos próprios, oriundos dos recursos garantidores do Plano de Benefícios 01, administrado pela PREVI.</p>
				<p align='justify'><b>				
										Condição suspensiva</b></p>
				<p align='justify'>
										3.1. Sem prejuízo das demais condições estipuladas neste Contrato, a entrega dos recursos do Financiamento ao VENDEDOR(ES) e ".$dev4.", no caso de ressarcimento de sinal, fica suspensivamente condicionada à comprovação à PREVI do efetivo e perfeito registro, junto à matrícula do Imóvel, da garantia de alienação fiduciária constituída nos termos da cláusula 13, o qual será realizado pelo Oficial de Registro de Imóveis competente. A comprovação do registro referido nesta cláusula dar-se-á pela entrega da ficha de matrícula atualizada do Imóvel.</p>
				<p align='justify'><b>								
										Verificação da condição suspensiva</b></p>
				<p align='justify'>
										3.2. Verificada a condição suspensiva estipulada na cláusula 3.1, a PREVI liberará os recursos referentes ao Financiamento, entregando-os diretamente ao VENDEDOR(ES), a título de pagamento do preço do Imóvel, observado sempre o disposto na cláusula 2.1.</p>
				<p align='justify'>				
										3.2.1. Caso parte do preço do Imóvel seja pago pela utilização de recursos do Fundo de Garantia por Tempo de Serviço – FGTS ".$dev3.", a PREVI liberará tais recursos diretamente ao VENDEDOR(ES) ou ao CREDOR QUITANTE, após satisfeita a condição suspensiva estipulada na cláusula 2.3, acima, ou após tê-los recebidos da Caixa Econômica Federal – CEF, o que ocorrer depois.</p>
				<p align='justify'><b>				
				...
				<p align='justify'>
										4.2. O índice de atualização monetária, definido no  item 3 do Quadro VII , incidirá sobre o Saldo Devedor antes da aplicação, sobre este, dos juros e encargos incorridos naquele mês, e antes da imputação dos pagamentos efetuados ".$dev5." naquele mês.</p>
				<p align='justify'>				
				...
				<p align='justify'>
										4.3. Caso o índice mencionado no item 3 do Quadro VII venha a ser substituído por outro índice de cunho nacional, e que se enquadre como referencial para a reavaliação atuarial do Plano de Benefícios 01 da PREVI, este novo índice será utilizado para efeito de atualização monetária, na forma e periodicidade estabelecida neste instrumento, a partir de sua adoção.</p>
				<p align='justify'>				
										4.3.1. No caso de substituição do índice mencionado no item 3 do Quadro VII, a PREVI divulgará, por meio de seus canais de comunicação institucionais, o novo índice a ser utilizado. </p>
				<p align='justify'><b>
										CLÁUSULA 5 – JUROS</b></p>
				<p align='justify'>
										Sobre o Saldo Devedor (conforme definido na cláusula 4.1.), acrescido da atualização monetária nos termos da cláusula 4, incidirão, desde a presente data, e com periodicidade mensal, juros à taxa efetiva mensal estipulada no item 5 do Quadro VII deste Contrato correspondentes aos juros atuariais e previstos no Regulamento do Plano de Benefícios 01, da PREVI, os quais deverão ser pagos mensalmente, conforme previsto na cláusula 7.</p>
				<p align='justify'><b>				
										Alteração da taxa efetiva de juros</b></p>
				<p align='justify'>
										5.1. Caso a taxa de juros atuarial do Plano de Benefícios 01 da PREVI, ao qual ".$dev2." encontra(m)-se vinculado(s), venha a ser alterada, esta nova taxa de juros será aplicada sobre o Saldo Devedor, nos termos da cláusula 4, a partir de sua adoção.</p>
				<p align='justify'>				
				...
				<p align='justify'>
										6.1 O Fundo de Liquidez será formado por contribuições mensais calculadas sobre o valor do saldo devedor atualizado,  pela aplicação do percentual definido no item 6.1 do Quadro VII,  e serão destinadas a quitar eventual resíduo do Saldo Devedor existente após o pagamento da última prestação, desde que o referido resíduo não tenha sido causado por inadimplemento ".$dev3."</p>
				<p align='justify'><b>				
										Fundo de Quitação por Morte</b></p>
				<p align='justify'>						
										6.2 O Fundo de Quitação por Morte (FQM) será formado por contribuições mensais calculadas sobre o valor do Saldo Devedor atualizado e em percentual definido em função da idade ".$dev3.", pela aplicação dos percentuais definidos no item 6.2 do Quadro VII e se destinará a quitar todas as obrigações vincendas em caso de morte ".$dev3."</p>
				<p align='justify'>
										6.2.1 - O percentual relativo ao FQM será alterado automaticamente,  durante a vigência do contrato,  em função da mudança de idade ".$dev3.", conforme definido no item 6.2 do Quadro VII. </p>
				<p align='justify'>				
										6.2.2 Havendo mais de um DEVEDOR o FQM quitará apenas as parcelas vincendas relativas ".$dev4." falecido(s), na proporção indicada no item 6.2 do Quadro XI. </p>
				<p align='justify'><b>				
				...
				<table border='1'  bordercolor='#000000'  cellspacing='0' width='600'>
					<tr>
						<td>
						    <font color='#FFFFFF'>LN</font><br><br>
							a=( b x ( 1 + d ) ) x ( 1/c + ( e + f ) )<br><br>
							Onde:<br><br>
							...
							f = taxa mensal equivalente do Fundo de Hedge<br>
						</td>
					</tr>
				</table>
				<br>
				...
				<p align='justify'>						
										7.5.1 Para ".$dev2." aposentado ou pensionista considera-se como proventos brutos a soma dos benefícios recebidos da PREVI e do INSS. Caso ".$dev2." não receba(m) os benefícios do INSS via folha de pagamentos da PREVI e na ausência de comprovação do benefício recebido por aquele Instituto, será utilizado para compor o total de proventos brutos, para os fins desta cláusula, o valor do teto vigente de benefícios definido pelo INSS.</p>
				<p align='justify'>						
										7.5.2 Para ".$dev2." aposentado ou pensionista que receba apenas o benefício do INSS pela PREVI, será considerado como proventos brutos, para fins da limitação de que trata esta cláusula, a renda bruta que serviu de base para a concessão do financiamento, devidamente atualizada pelo índice previsto na cláusula 4.</p>
				<p align='justify'>						
										7.5.3 A".$dev2." que recebe da PREVI a antecipação da complementação de aposentadoria, sem o benefício do INSS, considera-se como proventos brutos apenas os valores pagos pela PREVI, até a concessão de aposentadoria pelo INSS, após este evento será aplicado o disposto na cláusula 7.5.1.</p>
				<p align='justify'>						
										7.5.4 Se ".$dev2.", por qualquer motivo, deixar de receber proventos do Banco do Brasil S.A., da PREVI ou do INSS será considerada como renda bruta, para fins da limitação prevista na cláusula 7.5 a renda bruta que serviu de base para a concessão do financiamento, devidamente atualizada pelo índice previsto na cláusula 4.</p>
				<p align='justify'>						
				...
				<p align='justify'>
										Caso ".$dev2." não pague(m), na data de seu vencimento, qualquer obrigação pecuniária, de qualquer natureza, principal ou acessória, serão devidos à PREVI: i) atualização monetária dos valores não pagos pelo índice previsto no item 3 do  Quadro VII;  ii) juros contratuais previstos no item 4 do Quadro VII; iii) multa não indenizatória de 2% (dois por cento) e juros moratórios de 1% a.m. (um por cento ao mês) sobre os valores em atraso  atualizados acrescidos dos juros definidos no item 4 do Quadro VII; e iv) despesas de cobrança e honorários advocatícios.</p>
				<p align='justify'>						
				...
				<p align='justify'>						
										V – se qualquer das declarações feitas ".$dev5." ou pelo VENDEDOR(ES) neste Contrato revelar-se errônea, enganosa, falsa ou inverídica;</p>
				<p align='justify'>						
										VI – se houver o descumprimento ".$dev5." de qualquer obrigação por ele(es) assumida neste Contrato, inclusive daquelas relativas à garantia de alienação fiduciária ora constituída;</p>
				<p align='justify'>						
				...
				<p align='justify'>						
										14.1. Enviado o termo de quitação aqui mencionado, fica ".$dev2." autorizado(s) a requerer(em), ao Oficial de Registro de Imóveis competente, o cancelamento do registro da propriedade fiduciária, com a respectiva restituição ".$dev4." da propriedade sobre o Imóvel.</p>
				<p align='justify'>						
				...
				<p align='justify'>						
										16.1. O primeiro público leilão será realizado no prazo máximo de 30 (trinta) dias, contados da data do registro da consolidação da plena propriedade em nome da PREVI. O preço mínimo de venda do Imóvel, nesse primeiro público leilão, equivalerá ao valor de avaliação do Imóvel, estipulado pelas Partes, no Quadro III, o qual será atualizado pela mesma taxa de atualização do Saldo Devedor constante da cláusula 4, e que poderá, a critério exclusivo da PREVI, ser revisto por meio de nova avaliação, realizada por companhia idônea a ser indicada pela PREVI, incluindo-se os custos de avaliação no Saldo Devedor</p>
				<p align='justify'>						
				...
				<p align='justify'>						
										16.4.1. Realizada a venda do Imóvel em qualquer dos públicos leilões, a PREVI, na qualidade de proprietário pleno e possuidor indireto do Imóvel, transferirá ao licitante vencedor toda a propriedade e posse que sobre ele exerce, bem como receberá o preço pago pelo licitante vencedor, e utilizará os valores para liquidar as Obrigações Garantidas acrescidas dos juros e encargos moratórios até a data da realização do leilão em que houve a venda do Imóvel e do valor das obrigações ".$dev3." descritas nos itens “i” a “x” da cláusula 16.2.1, que se encontrem vencidas e não pagas até a data da efetivação da venda do Imóvel.</p>
				<p align='justify'><b>						
				...
				<p align='justify'>						
										Caso no Quadro I - “COMPRADOR(ES)”, constante do preâmbulo deste Contrato, figurem duas pessoas, ambas declaram-se solidariamente responsáveis por todas as obrigações decorrentes do Financiamento e descritas neste Contrato, entendendo-se as referências feitas neste Contrato ao “DEVEDOR” como abrangendo ambas as referidas pessoas, as quais, mútua e reciprocamente, constituem-se procuradoras uma(s) da(s) outra(s) para fins de receber citações, intimações e interpelações de qualquer procedimento, judicial ou extrajudicial, decorrente deste Contrato, inclusive as intimações mencionadas na cláusula 15, de modo que, realizada a citação ou intimação, na pessoa de qualquer uma delas, estará completo o quadro citatório.</p>
				<p align='justify'><b>									
				...
				<p align='justify'><b>									
										CLÁUSULA 21 - AUTORIZAÇÃO PARA REGISTROM</b></p>
				<p align='justify'>						
				...
				<p align='right'><br><br>								
										".data_extenso($asscontrato).".</p>
				<br>
				<p align='justify'>COMPRADOR(ES):</p><br>
				".$assinatura_dev."
				<br>
				<p align='justify'>VENDEDOR(ES):</p><br>
				".$assinatura_vend."
				<br>
				...
				<tr>
					<td width='600'></td>
					<td width='364' align='center'><b>P.P. CAIXA DE PREVIDÊNCIA DOS FUNCIONÁRIOS DO BANCO DO BRASIL - JOSÉ AUGUSTO FERREIRA GOMES</b></td>
				</tr>
			</table>
				<br>
